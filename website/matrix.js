...
const versions = ({ index, lines }, { hydra, keto, oathkeeper }) => {
  ...
  const updated = columns.slice(3, 6).map((column, index) => versions[index] ? [
      ...column.split(',').map((v) => v.trim()).filter(p => p.length > 0),
      '`'+versions[index]+'`'
    ] : column
  ).filter(onlyUnique).map(inner => inner.join(', '))
...
Promise.all(services.map((service) => fetch(`https://arekkas:${process.env.GITHUB_TOKEN}@api.github.com/repos/ory/${service}/releases`)
  .then(res => res.json())
