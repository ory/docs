...
const enhance = (schema, parents = []) => (item) => {
    ...
    const path = [...parents.map((parent) => ['properties', parent]), ['properties', key]].flat()
    ...
    if (examples) {
        comments.push(' Examples:', ...YAML.stringify(examples).split('\n').map((i) => ` ${i}`)) // split always returns one empty object so no need for newline
    }
...
new Promise((resolve, reject) => {
    ...
    const removeAdditionalProperties = (o) => {
        ...
        if (o.properties) {
            Object.keys(o.properties).forEach((key) => removeAdditionalProperties(o.properties[key]))
        }
    }

    removeAdditionalProperties(schema)
    if (schema.definitions) {
        Object.keys(schema.definitions).forEach((key) => removeAdditionalProperties(schema.definitions[key]))
    }

    const values = jsf.generate(schema)
    const doc = YAML.parseDocument(YAML.stringify(values))

    const comments = [`# ${pathOr(process.argv[2].split('/').pop(), ['title'], schema)}`, '']
