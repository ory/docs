...
description: Scrape Ory metrics from each service's Prometheus-compatible endpoint and visualize them in Grafana.
...
[self-hosted observability](../../self-hosted/operations/15_observability.md) docs document this as the recommended metrics path.
...
For richer signals, pair with the [OpenTelemetry integration](./opentelemetry.mdx) for traces and a backend like Tempo.
