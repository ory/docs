...
configurable templates (document capture, selfie + liveness, database checks,
watchlist screening). This integration runs a Persona Inquiry from Ory Actions
during registration, consumes Persona's HMAC-signed callback when the inquiry
completes, and gates login on the stored verification status.
