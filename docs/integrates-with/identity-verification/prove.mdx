...
id: prove
title: Prove
description: Use Prove's phone-anchored identity signals from Ory Actions for prefill, enrollment, and per-login validation.
sidebar_label: Prove
---
...
[Prove](https://www.prove.com) (formerly Payfone) is a phone-anchored identity platform — telecom signals, SIM-swap detection,
identity prefill, and continuous phone-ownership verification. This integration calls Prove from Ory Actions across three
lifecycle points: pre-registration lookup (prefill), post-registration enrollment, and per-login validation.
...
1. Use [`ory-actions.yaml`](https://github.com/ory/integrates/tree/main/identity-verification/prove/ory-actions.yaml) as the
   snippet for `registration.before` (lookup), `registration.after` (enroll), and `login.after` (validate).
2. The body templates are in [`jsonnet/`](https://github.com/ory/integrates/tree/main/identity-verification/prove/jsonnet).
3. **Phone collection is a prerequisite** — your identity schema must capture a phone number in E.164 format before the
   `prove/lookup` hook fires; surface the field on your registration UI.
