...
[Ory Network](https://console.ory.sh/) uses cloud-native open-source technologies (Kubernetes, Crossplane, Cockroach, Linux,
Istio, Ory) and standards (OAuth 2.0/2.1, OpenID Connect, MITREid, WebAuthn, TOTP, FIDO3) to deliver a low-latency, planet-scale
...
[Ory Network](https://console.ory.sh)'s core infrastructure components are open source to foster collaboration, reduce supply
...
- Ory OAuth2 & OpenID implements 15+ IETF and OpenID standards to faciliate single-sign on, delegation and API access
...
- Ory Permissions is a low-latency, high-performace, relationship-based authorization system that enables fine-grained access
...
Ory Network is the commercial offering of Ory and is built on top of our popular open-source software. Our goal with Ory Network
is to offer a planet-scale, low-latency, resilient, and secure service that's easy to use and set up.

In short: Ory Network is the most convenient and straightforward way to run Ory. Sign up at
[console.ory.sh](https://console.ory.sh/registration) for an account and create a free developer project.

### Components

Ory Network uses a variety of components - some are based on open-source projects and others are closed source.

### Console

The Ory Console is the management UI of Ory Network. It offers capabilities such as showing and searching for identities, managing
...
Ory Network incorporates the open-source [Ory Kratos Identity Server](https://www.ory.sh/kratos) project and offers:
...
Ory Network incorporates the open-source [Ory Keto Permission Server](https://www.ory.sh/keto) project and offers:
...
**No!** Ory Network is our commercial offering with which we generate revenue to build more open source and more features in Ory
Cloud. Our value proposition and vision for Ory Network is to incorporate and advance the open-source ecosystem and add additional
services which aren't possible to open source (for example Analytics, SLAs, SRE, Low-Latency).

Ory Network and our open-source software are interlinked. While we've made some modifications to the open source to make it work
better in our multi-tenant environment and operational (SRE) model, we're using the same code base, APIs, features, and
configurations for Ory Network.
...
only impact Ory Network will have is that it will accelerate development and enable us to spend more resources on the open source!
...
- [Sign up](https://console.ory.sh/registration) for a free Developer account in Ory Network.
...
- The best way to run Ory: Learn about [Projects](https://www.ory.sh/docs/concepts/project) and
...
- [Ory Network Frontend Example](https://www.youtube.com/watch?v=VKLXbrZS7tU)
- [Ory Network Backend Example](https://www.youtube.com/watch?v=umSRQBVdPjM)
