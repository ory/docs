...
Ory offers a powerful session management system that is based on cookies for browser clients and API tokens
for API clients. Additionally, Ory supports converting a session to a JSON Web Token (JWT). A JWT might be the better option in situations, such as:
