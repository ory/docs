...
Sometimes, Jsonnet scripting is not enough to customize the JWT. A network call to a different service is necessary.
A typical use case is adding custom claims to the tokens based on a separate database or business logic.

This is possible by registering a webhook endpoint in the configuration. Before the token is issued to the
client, Ory will call your HTTPS endpoint with information about the client requesting the token.


:::note
Be aware of the added request latency, and of the impact of this service not being available: authentication will fail for the user.
:::
