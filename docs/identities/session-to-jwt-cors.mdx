...
curl -X GET \
  ...
  "https://{project.slug}.projects.oryapis.com/sessions/whoami?tokenize_as=jwt_example_template1"
...
- `claims`: The default claims the token has per default. You can modify these claims but not remove them:
  ...
  - `iss`: The project's slug url (`https://{project.slug}.projects.oryapis.com`).
