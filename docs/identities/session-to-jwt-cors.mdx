...
This is possible by registering a webhook endpoint in the configuration. This is all done in a separate endpoint: `/sessions/whoami-jwt/{templateName}`.
Before the token is issued to the client, Ory will call your HTTPS endpoint with information about the client requesting the token. Note that the template name is mandatory.
This way, your application can at runtime use different template names and thus third-party endpoints at will.
...
- Added request latency: if the third-party service takes 100 milliseconds to responsd to the request, that means that the
  response time for the `/sessions/whoami-jwt/{templateName}` increases by that much
