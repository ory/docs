...
This is enabled by turning a feature flag to 'on' at the project level (opt-in). This flag is enabled by default for newly created
projects.
...
If you do have a phone number and want to receive a SMS containing the recovery code, make sure that this phone number is
registered as a recovery address.

If you have multiple recovery addresses registered, during the recovery flow, you will be presented a list of your (masked)
addresses and you will be prompted to choose one of them.
...
It is safe to toggle this feature flag on and off. This flag only is take into account when a new recovery flow is created.
Existing flows will be unaffected by toggling the flag.
...
When integrating with the API directly, your applications must send and receive these fields. Refer to the OpenAPI specification
for more details.
