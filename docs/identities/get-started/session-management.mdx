...
function RefreshSessionButton() {
  return (
    <button onClick={refreshUserSession}>
      Refresh Session
    </button>
  )
}
...
- Allow users to [change sensitive settings](/docs/kratos/session-management/session-lifespan#privileged-sessions) (like passwords, adding a second factor, or changing email) in their profile.
