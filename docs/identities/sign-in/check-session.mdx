...
:::warning Edge Sessions need the `__cflb` cookie

When using Edge Sessions to serve `/sessions/whoami` checks with 40ms latency, make sure to pass the `__cflb` cookie as well.
