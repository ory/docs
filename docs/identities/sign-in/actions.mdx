...
```shell
ory patch identity-config {project_id} \
  --add '/selfservice/flows/registration/after/hooks=[{"hook": "show_verification_ui"}]'
```
...
For SPA & native clients, the response from the registration endpoint contains a `continue_with` field. This field lists possible
actions, the user might need to or can take next. For example, an object containing the ID of the verification flow created as
...
The response to the registration endpoint always contains the `continue_with` field with the `verification_ui` action if
verification is enabled and the identity schema defines at least one verifiable address. Due to this, the `show_verification_ui`
hook does nothing for API & SPA clients.
