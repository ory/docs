...
- their email domain does not match a certain value - this way you can allow only users from certain organizations to sign up
- their IP address is not allowed - this way you can prevent user registrations from locales where you don't provide services
...
Read the [Flow-interrupting webhooks documentation](../../kratos/hooks/01_configure-hooks.mdx#flow-interrupting-webhooks) to learn
more.

## Revoke other sessions at login
...
Run this command to revoke all active session's of the user after every login:
...
To use this feature only for specific method(s), run this command:
...
2. Check the JSON output and identify the array index of the `revoke_active_sessions` hook:
   ...
   ```json
   {
     "hooks": [
       ...
       {
         "hook": "revoke_active_sessions" // The index of this hook is '0'.
       }
     ],
     "oidc": {
       "hooks": []
