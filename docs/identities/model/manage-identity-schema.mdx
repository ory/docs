...
It's recommeneded to manage identity schemas in version control. Learn more about
...
1. Find the identity to be updated and note down their `id`. Save the full identity to a file using the following command:
...
1. Assess the required updates in traits. Remove the credential object, since you don't want to change credentials. You then need
   to add, remove, or update the traits to match the new identity schema. You also need to change the `schema_id` and `schema_url`
   to the new ones. For instance, adding two new traits and removing an old one:
...
1. In order update the identity, you need to use the
   [Admin API](https://www.ory.sh/docs/reference/api#tag/identity/operation/updateIdentity). The API requires the Ory Network
   [Project slug](https://console.ory.sh/projects/current/developers/guides) and
   [API Key](https://console.ory.sh/projects/current/developers). Set them as environment variables:
...
1. Update the identity using the following command:
...
This should return the modified identity as response. Now, you have migrated a single identity to a new identity schema. If you
have more identities to be patched to the new schema, repeat the above process for each of them.
