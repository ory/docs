...
const (
	YOUR_ORY_API_KEY        = "{api-key}" // Add your API key here
	YOUR_ORY_PROJECT_SLUG   = "{project-slug}" // Add your project slug here
	UPDATE_TO_NEW_SCHEMA_ID = "{new-schema-id}" // Add the new schema ID here
	IDENTITY_ID_TO_UPDATE   = "{identity-id}" // Add the identity ID to update here
)
