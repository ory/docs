...
You must define which schemas are available for self-service selection by using the `selfservice_selectable` flag. Only schemas
with this flag set to `true` can be selected through the `identity_schema` query parameter.
