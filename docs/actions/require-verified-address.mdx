...
If the identity has no verified addresses, a code will be sent to the first email or phone number in the list.
...
When a user updates their email or phone number, Ory sends a verification code to the new address. Per default, the settings flow
will show a success error when the profile is updated.

### Show verification screen after address change

To show the verification screen when the user changes one of their verified addresses instead of a success message, use the Ory
CLI:
