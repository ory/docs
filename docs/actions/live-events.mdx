...
Per the JSON specifiction, ordering of keys in objects is not guaranteed and should not be relied upon.

New keys can appear at any time and consuming code should be ready for it.

The exact keys in the `eventAttributes` fields are currently not documented and not considered stable (i.e. no backwards
compability guarantees are made at the moment for them). If you intend to write a consumer that needs a detailed schema and
...
1. If a request being served is suddenly interrupted (e.g. because of a crash), events up to that point for this request will
   still be emitted. That means that a request might have missing events (e.g. `LoginInitiated` _not_ followed by any subsequent
   event).
2. There might be duplicates in the event stream. While rare (typically caused by retries because of network issues), consumers
   should be prepared to deal with them, for example by storing the events in a database for deduplication.
3. Some events might not emitted at all and be lost (see point 1.). This is deemed to be rare and can only happen in the face of
   special adverse conditions. In other words, delivery is currently 'best-effort' and not 'at-least-once'.
4. We do not currently offer SLAs for the event delivery duration (how much time elapses between when the event happens, and when
   it is visible in your consumer or in the console UI). However we typical observe < 5 seconds delivery times.
5. Ordering: We guarantee neither the ordering of events for different requests, nor events within one request. That means that if
