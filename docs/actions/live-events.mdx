...
```json
{
  ...
  "version": int64,     // The schema version of the event.
  "timestamp": string,  // ISO 8601 formatted timestamp when the event occurred (e.g., "2025-05-16T11:18:56Z").
...
and are delivered in the corresponding JSON format.
...
- The exact content in the `eventAttributes` field is currently not documented and not considered stable. That means that no backwards
compability guarantees are made at the moment for them.


If you intend to write a consumer that needs a detailed schema and
stability guarantees about the `eventAttributes` field, please [reach out](https://www.ory.sh/contact/).
...
1. **Partial delivery:** If a request being served is suddenly interrupted, for example because of a crash, events up to that point for this request
   will still be emitted. That means that a request might have missing events (for instance, `LoginInitiated` _not_ followed by
   any subsequent event).
2. **Possible duplicates:** There might be duplicates in the event stream. While rare (typically caused by retries because of network issues), consumers
   should be prepared to deal with them, for example by storing the events in a database for deduplication.
3. **Best-effort delivery:** Some events might not emitted at all and be lost (see point 1.). This is deemed to be rare and can only happen in the face of
   special adverse conditions. In other words, delivery is currently 'best-effort' and not 'at-least-once'.
4. **No delivery latency SLA:** We do not currently offer SLAs for the event delivery duration (how much time elapses between when the event happens, and when
   it is visible in your consumer or in the console UI). However we typical observe shorter than 5 seconds delivery times.
5. **No ordering guarantees:** We guarantee neither the ordering of events for different requests, nor events within one request. That means that if
   first request is made, resulting in events `a` and `b`, and then a second request is made, resulting in events `c` and `d`, any
   of the `4! = 24` permutations of `abcd` are to be expected.

If stronger guarantees are required for your use case, please [reach out](https://www.ory.sh/contact/).
...
exponential backoff between attempts. This may lead to duplicates in the event stream, so consumers should be prepared to deal
with them.
