...
1. **Possible duplicates:** There might be duplicates in the event stream. While rare (typically caused by retries because of
   network issues), consumers should be prepared to deal with them, for example by implementing deduplication logic.
1. **Best-effort delivery:** Event delivery is best-effort. While most events are delivered reliably, events may be lost under
   adverse conditions (for example, system crashes during request processing). Delivery is not guaranteed to be "at-least-once".
1. **No delivery latency SLA:** We do not offer a Service Level Agreement (SLA) for event delivery duration (the time between an
