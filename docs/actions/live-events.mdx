...
by this schema and are delivered in the corresponding JSON format:
...
The exact keys in the `event_attributes` fields are currently not documented and not considered stable (i.e. no backwards
compability guarantees are made at the moment for them). New keys can appear at any time and consuming code should be ready for it.

If you intend to write a consumer that needs a detailed schema and stability guarantees about these fields, please
[reach out](https://www.ory.sh/contact/).

## Event names

The following is a list of all events that are currently supported. The first column corresponds to the `name` field in an event. Names are case-sensitive.
...
- There might be duplicates in the event stream. While rare, consumers should be prepared to deal with them, for example by
  storing the events in a database for deduplication.
- Some events might not emitted at all and be lost. This is deemed to be rare and can only happen in the face of special adverse
  conditions. In other words, delivery is currently 'best-effort' and not 'at-least-once'.
- We do not currently SLAs in place for the event delivery duration (how much time elapses between when the event happens, and
