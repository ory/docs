...
Live event streams are available for Ory Network enterprise contracts. Talk to your account manager or [reach out directly](https://www.ory.sh/contact/).

## Event descriptions

Ory emits events for many different actions. The following is a list of all events that are currently supported. All events abide by this schema and are delivered in the corresponding JSON format:
...
```json
{
  ...
  "timestamp": "2025-05-16T08:58:45Z",
  "attributes": [
    {
    ...
    {
      "key": "GeoLocationCountry",
      "value": "DE"
    },
    {
      "key": "GeoLocationRegion",
      "value": "BY"
    },
    {
      "key": "WebhookID",
    ...
    {
      "key": "WebhookTriggerID",
      "value": "67f5486a-ded4-4c3f-bae1-585192dfa0e7"
    }
  ]
}
```

The exact keys in the `event_attributes` fields are currently not documented and not considered stable (i.e. no backwards compability guarantees are made at the moment for them).

If you intend to write a consumer that needs a detailed schema and stability guarantees about these fields, please [reach out](https://www.ory.sh/contact/).
...
- There might be duplicates in the event stream. While rare, consumers should be prepared to deal with them, for example by storing the events in a database for deduplication.
- Some events might not emitted at all and be lost. This is deemed to be rare and can only happen in the face of special adverse conditions. In other words, delivery is currently 'best-effort' and not 'at-least-once'.
- We do not currently SLAs in place for the event delivery duration (how much time elapses between when the event happens, and when it is visible in your consumer or in the console UI).
  However we typical observe < 5 seconds delivery times.
...
exponential backoff between attempts. This may lead to duplicates in the event stream, so consumers should be prepared to deal with them.
