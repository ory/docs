...
The exact keys in the `eventAttributes` field are currently not documented and not considered stable. That means that no backwards
...
1. If a request being served is suddenly interrupted, for example because of a crash, events up to that point for this request
