...
Ory emits events for many different actions. All events abide
by the same schema:
...
The following is a list of all events that are currently supported. The first column corresponds to the `name` field in an event. New event names may be added in the future, in which case this list will be updated accordingly.
...
1. If a request being served is suddenly interrupted (e.g. because of a crash), events up to that point for this request will still be emitted.
  That means that a request might have missing events (e.g. `LoginInitiated` *not* followed by any subsequent event).
2. There might be duplicates in the event stream. While rare (typically caused by retries because of network issues), consumers should be prepared to deal with them, for example by
  storing the events in a database for deduplication.
3. Some events might not emitted at all and be lost (see point 1.). This is deemed to be rare and can only happen in the face of special adverse
  conditions. In other words, delivery is currently 'best-effort' and not 'at-least-once'.
4. We do not currently offer SLAs for the event delivery duration (how much time elapses between when the event happens, and
  when it is visible in your consumer or in the console UI). However we typical observe < 5 seconds delivery times.
5. Ordering: TODO
