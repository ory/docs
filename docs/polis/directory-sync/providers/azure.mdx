...
If you map a custom attribute and it doesn't appear in the user record in Ory Polis, the cause is almost always on the Microsoft Entra ID side. Ory Polis stores every attribute it receives under the schema it arrives in, so a missing attribute means Entra ID didn't send it. 
Check the following in Microsoft Entra ID:
