...
To configure these values with the Ory CLI use:
...
SMTP has six different security mechanisms. Most SMTP services today use Explicit StartTLS with trusted certificates - the item in
the list below.
...
1. **Recommended**: StartTLS with certificate trust verification. This is the most common option today:
   ...
   ```
1. StartTLS without certificates trust verification:
   ```
   smtp://username:password@server:port/?skip_ssl_verify=true
   ```
1. Cleartext SMTP uses no encryption and is not secure. This option is often used in development environments:
   ```
   smtp://username:password@server:port/?disable_starttls=true
   ```
1. Implicit TLS with certificates trust verification:
   ```
   smtps://username:password@server:port/
   ```
1. Implicit TLS without certificates trust verification:
   ```
   smtps://username:password@server:port/?skip_ssl_verify=true
   ```
1. Implicit TLS with certificate verification which works if the server is hosted on a sudomain and uses a non-wildcard domain
   certificate:
   ```
...
In the future, we will improve error observability and transparency for this feature. If you have problems with email delivery
over custom SMTP servers, please, for the time being, reach out to us on [Slack](https://slack.ory.sh) or
[GitHub](https://github.com/ory/cloud) for assistance.
