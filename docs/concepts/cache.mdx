...
Edge session caching is an Ory Network feature designed to ensure the lowest possible latency on the critical-path Check Session
API, which includes the `toSession` and `/sessions/whoami` endpoints.

:::tip

Edge sessions are available on all paid plans. You must enable this feature manually. Read the [Configuration](#configuration)
section to learn how to turn it on.
...
By default, edge sessions are turned off for all projects. Follow these steps to turn the feature on:
