...
:::warning Don't forget the `__cflb` cookie

When calling the `/sessions/whoami` endpoint ensure to also pass the `__cflb` cookie to it, or you get a lot of cache misses.
