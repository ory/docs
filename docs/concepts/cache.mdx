...
The Ory Network provides a global Edge Network to ensure low-latency access to critical-path APIs. These APIs are eventually
consistent, which means that the changes you make are always reflected in the API, even if in some cases this might not be
instantaneous.
...
To configure session caching, go to **Customize** → **Advanced Settings** in the
