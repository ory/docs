...
Ory Cloud has a total of six flows, **Login**, **Registration**, **Verification**,
**Recovery**, **Settings** and **Logout** which can be configured to redirect back to any
URL. A common use case would be to redirect the user to your application
home screen after a logout or to a specific url on a sub-domain after a settings
password update. On project creation the redirect flows are set to the
...
Setting `Post-OIDC` redirects for Login and Registration will be supported in
a future release. For more information see the [Ory Cloud Roadmap](../cloud/cloud-roadmap.mdx).
