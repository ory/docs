...
`return_to` is set when the login flow is started in a browser with a
...
in the example below, the Post-Login Default Redirect URL is set to a path.
This means the base URL will be the Global Redirect URL (`https://myapp.com`).
This will set the Login Default Redirect URL to
...
In the example below , the Post-Login Default Redirect URL is set to use a
FQDN. This will set the Login Default Redirect URL to
`https://myapp.com/after/login` and the Post-Password to
`https://myapp.com/after/login/password`.
