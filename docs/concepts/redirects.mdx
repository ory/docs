...
PHP, Golang etc.) and how to configure them. To learn more about the SDK for
Single Page Applications, check out the [SDK](./services-api.mdx) documentation.

## Allow List

Set dynamic redirects using the `?return_to=` query parameter on
[Self-Service Flows](./self-service.mdx). For example: a user opens a sharable
link to go to `https://myapp.com/posts`. This URL requires the user to have an
active session and redirects the user back to the login page. To return the user
back to the original URL, append `?return_to=https://myapp.com/posts` when
starting the
...
Cloud [Managed User Interface](./managed-ui.mdx).
