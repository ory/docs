...
to configure it on Ory Cloud console. If you are interested in utilizing the
...
Setting `Post-OIDC` redirects for Login and Registration are not
supported since an Ory Cloud project can't be setup to use OIDC yet. This will
...
for example a path such as `/login` will alter `https://domain.example/global/path` to
...
The `Post-Recovery` redirect is not supported and will automatically
redirect the user to the `Settings UI` URL. This implies that you should rely on the `Post-Settings` redirect URL for `Post-Recovery` flows as
well.
...
A redirect can be updated or reset by either changing the current
...
the default value which is Ory Cloud
