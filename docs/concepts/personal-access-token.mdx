...
[//] #1. Go to the [Ory Console](https://console.ory.sh/). 2. Go to **Workspace settings** through the workspace menu in the top
left corner. 3. Select **API keys** in the sidebar. 4. Click the **+** icon in the **API Keys** section. 5. Enter the API Key
name 6. Select an **expiration** date. 7. Click **Create** 8. Copy the created API Key from the prompt that shows at the bottom
right of the screen.
