...
[`/sessions/whoami`](../../reference/api#sessionWhoami) endpoint will return with an error with an id `session_aal2_required` if
the user is required to complete a second factor. More information on UI error messages can be found on the
[Customize user interface](../kratos/concepts/ui-user-interface) page.
...
Registration data are dependent on the [Identity Schema](../identities/manage-identity-schema) traits. This ensures registration
