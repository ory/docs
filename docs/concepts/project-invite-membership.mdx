...
doesn't depend on the user having an Ory Cloud account. Ory Cloud will send
...
match the email specified in the invite, if they don't match, the user will
...
It's currently not possible to elevate a member from `developer` to `owner`,
...
Once an invite has been removed by the project owner, the invite will
