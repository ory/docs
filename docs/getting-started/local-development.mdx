...
ideal for early-stage development and testing and should not be used to store PII. To create a new development project, use the
following command:
...
`https://auth-staging.example.com`). Staging projects have lower rate limits than production projects and should not be used to
store PII.
