...
- Your local machine is usually available on both `localhost` and `127.0.0.1`. For the browser these two domains are different
  top-level-domains. Cookies set on `localhost` will not be sent in requests to `127.0.0.1` and vice versa. If you encounter
  cookie issues, make sure that all of your requests are sent to `localhost` and not `127.0.0.1`.
