---
id: ory-cloud-oauth2
title: Perform OAuth2 Authorization Code Grant with Ory Network
sidebar_label: Federation with OAuth2
---

This guide shows you a sample implementation of the
[Authorization Code Grant](https://www.rfc-editor.org/rfc/rfc6749#section-1.3.1) OAuth2 flow using Ory OAuth2 & OpenID in Ory
Network. It takes you on a complete journey, from initial setup to running an OAuth2 flow.

:::tip

Ory OAuth2 & OpenID (based on [Ory Hydra](../hydra/index.md)) is available in Ory Network out of the box, which means you can use
...
- Create an OAuth2 client in Ory Network.
...
- Configure Ory Network to work with the sample application that runs on your machine.
- Perform Authorization Code Grant in Ory Network using the login and consent app and a sample web server acting as an OAuth2
  Client.
...
- Create an Ory Network account and project. Use [Ory CLI](../guides/cli/cli-basics#create-ory-cloud-project) or go to
...
Create a new OAuth2 client in Ory Network by sending a POST request to the `/admin/clients` API. This is a protected endpoint,
which means that you must include the Ory PAT in the `Authorization` header of the request.

Alternatively, you can use the Ory CLI and pass your Ory Network project slug using flags.
...
<Tabs>
  <TabItem value="orycli" label="Ory CLI">

```shell
ory create oauth2-client \
  --project ORY_CLOUD_PROJECT_SLUG_OR_ID \
  --name YOUR_CLIENT_NAME \
  --grant-type authorization_code,refresh_token \ # Mandatory for this example, don't change.
  --response-type code,id_token \ # Mandatory for this example, don't change.
  --scope openid,offline \ # Mandatory for this example, don't change.
  --redirect-uri http://127.0.0.1:5555/callback # Mandatory for this example, don't change.
```
  </TabItem>
  <TabItem value="curl" label="cURL" default>

```bash
curl -X POST 'SDK_CONFIGURATION_URL/admin/clients' \  # Found in the 'Connect' section of the Ory Console. Must end with '/admin/clients'.
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer ORY_PAT' \  # Ory PAT copied from the Ory Console, 'ory_pat' prefix included.
  -d '{
...
By default, Ory Network uses internal redirect URLs for operations such as user login and consent. You can adjust these URLs to
...
Use the Ory CLI to create a sample web server that acts as the OAuth2 client. To successfully perform the Authorization Code Grant
flow, the client ID and client secret must be registered in Ory Network.
