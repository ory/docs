...
Alternatively, you can use the Ory CLI and pass your Ory Cloud project slug using flags.
...
Leave all parameters untouched except for `ORY_CLOUD_PROJECT_SLUG_OR_ID`, `YOUR_CLIENT_NAME`.
...
* `SDK_CONFIGURATION_URL`: Found in the 'Connect' section of the Cloud Console. Must end with '/admin/clients'.
* `ORY_PAT`: Ory Personal Access Token copied from the Cloud Console, 'ory_pat' prefix included.
...
flow, the client ID and client secret must be registered in Ory Cloud.
