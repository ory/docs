...
sidebar_label: OAuth2 in Ory Cloud
---

This guide shows you a sample implementation of an Authorization Code Grant OAuth2 flow in Ory Cloud that you can use as a
starting point for using the Ory Cloud OAuth2 Service (Ory Hydra). It takes you on a complete journey, from initial setup to
running an OAuth2 flow.

:::tip

Ory Cloud supports OAuth2 out of the box, which means you can use OIDC, Authorization Code Grant, Client Credentials Grant, and
more. Read [this document](../hydra/concepts/before-oauth2) to learn more about OAuth2 and evaluate if it's the right fit for you.
...
1. Download the configuration of your Ory Cloud project and save it to a `yaml` file:
...
3. Update the Ory Cloud project configuration using the file you worked with:
   ...
   ```shell
   ory update project <project-id> --file config.yaml
   ```
