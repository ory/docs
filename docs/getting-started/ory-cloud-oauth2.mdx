...
[Authorization Code Grant](https://www.rfc-editor.org/rfc/rfc6749#section-1.3.1) OAuth2 flow using Ory OAuth2 & OpenID Connect in Ory
Network. It takes you on a complete journey, from initial setup to running an OAuth2 flow.

:::tip

Ory OAuth2 & OpenID Connect (based on [Ory Hydra](../hydra/index.md)) is available in The Ory Network out of the box, which means you can
use OIDC, Authorization Code Grant, Client Credentials Grant, and more. Read [this document](../hydra/concepts/before-oauth2) to
learn more about OAuth2 and evaluate if it's the right fit for you.
