---
id: ory-cloud-oauth2
title: Perform OAuth2 Authorization Code Grant with The Ory Network
sidebar_label: Federation with OAuth2
---

This guide shows you a sample implementation of the
[Authorization Code Grant](https://www.rfc-editor.org/rfc/rfc6749#section-1.3.1) OAuth2 flow using Ory OAuth2 & OpenID Connect in
Ory Network. It takes you on a complete journey, from initial setup to running an OAuth2 flow.

:::tip

Ory OAuth2 & OpenID Connect (based on [Ory Hydra](../hydra/index.md)) is available in The Ory Network out of the box, which means
you can use OIDC, Authorization Code Grant, Client Credentials Grant, and more. Read
[this document](../hydra/concepts/before-oauth2) to learn more about OAuth2 and evaluate if it's the right fit for you.
...
- Perform Authorization Code Grant in The Ory Network using the login and consent app and a sample web server acting as an OAuth2
  Client.
...
- Create an account and project in The Ory Network. Use [Ory CLI](../guides/cli/cli-basics#create-ory-cloud-project) or go to
...
Create a new OAuth2 client in The Ory Network by sending a POST request to the `/admin/clients` API. This is a protected endpoint,
...
flow, the client ID and client secret must be registered in The Ory Network.
