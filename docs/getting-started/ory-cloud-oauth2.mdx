...
- Using the Authorization Code Grant (with OpenID) flow with user accounts created in Ory Identities and login and consent screens
  supplied by the Ory Account Experience.
- Using the Client Credentials Grant in The Ory Network
...
Make sure you're running the latest version of the CLI. Run:
...
The [Authorization Code Grant](https://www.rfc-editor.org/rfc/rfc6749#section-1.3.1) is most commonly used in scenarios where
...
To achieve that, the online marketplace must access the user's Google Photos library on their behalf.

If the user accepts the access scope requested by the app (online marketplace), the app's client gets an `id_token` and
`access_token` pair which is then used to interact with Google Photos.
...
communications. This allows web services, applications, or devices to call other entities using their own identities without
impersonating any human users. Activity that uses this grant often runs in the background and doesn't require any interaction from
users.
