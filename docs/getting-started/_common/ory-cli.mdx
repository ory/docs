...
To make Ory APIs and your application available on the same domain, Ory Tunnel mirrors Ory endpoints and rewrites cookies to
match the correct domain. As a result, the domain of the cookies is set correctly to the domain you run the app on instead of
`<your-project-slug>.projects.oryapis.com`.

By using the Tunnel, you can easily connect the application you're developing locally to Ory Network and consume the APIs
without additional configuration or self-hosting any Ory services.
