...
3. Run the Ory tunnel to expose Ory API under the same top-level domain as your application (`localhost`):

```shell-session
ory tunnel --port 3005 http://localhost:4005
```

4. Open [http://localhost:4005](http://localhost:4005) to access the application. As the initial call is made by an
