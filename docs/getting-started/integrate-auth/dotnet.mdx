...
1. Create an ASP.NET Core app. We will base this on the empty web template. Run these commands:
...
2. Run this command to install the Ory SDK which allows you to interact with Ory APIs:
...
```csharp title="Program.cs" showLineNumbers {4-9}
...
```csharp title="Program.cs" showLineNumbers {11-41}
var builder = WebApplication.CreateBuilder(args);
var app = builder.Build();
...
1. Create the `Index` RazorPage that renders the page with the session data:
...
3. To configure the app to serve RazorPages, make the following changes in `Program.cs`:

```csharp title="Program.cs" showLineNumbers {3-4,21-22}
var builder = WebApplication.CreateBuilder(args);
