...
prepared a sample React Native application that implements screens for:
...
## Creating the Application with Expo

To create the sample application, you can use Expo and the Ory React Native templates.

:::note

To follow these steps, you must [install Node.js and npm](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm).

:::

Follow these steps:

1. Run these commands to install Expo and create a new project using the Ory templates:

  ```shell
  # Install Expo
  npm install -g expo-cli
  # Create a new project using Ory templates
  expo init login-signup-app -t @ory/expo-login-registration-template --npm
  ```

2. Next, install and initialize all required components. Run:
...
3. At this point, the application is ready for you to explore! Use these commands to run the application in the preferred way:

  - `npm start` opens a dashboard where you can choose to run the app in iOS, Android, or web environments.
  - `npm run android` runs the app in the Android environment (requires a connected Android VM or device).
  - `npm run ios` runs the app in the iOS environment iOS (available only on macOS).
...
1. Sign in at [console.ory.sh](https://console.ory.sh/), select **Connect** from the left navigation panel, and copy the URL from the **SDK Configuration**
   section.
...
With this setup in place, the application can store and refresh the user session. Additionally, this allows the app to verify if the user session is
still active in the navigation and show the dashboard or login/registration screens accordingly:
