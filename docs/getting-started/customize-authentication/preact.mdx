---
id: preact
title: Learn how to create custom UI for an SPA with Ory Elements
sidebar_label: Create custom UI with Ory Elements
...
This example provides a closer look at integrating Ory Elements with a Preact TypeScript single-page application (SPA). By
following the instructions you will:

- Create a simple Preact Typescript SPA.
- Install Ory Elements and Ory SDK as dependencies.
- Create UI views for the application with Ory Elements them and allow the UI to consume Ory APIs through the Ory SDK.
...
edit your project's [Identity Schema](../../kratos/manage-identities/identity-schema) or try changing certain settings such as
allowing [passwordless](https://www.ory.sh/docs/kratos/passwordless/webauthn) flows.
...
To learn more about the self-service registration flow, read
[this document](https://www.ory.sh/docs/kratos/self-service/flows/user-registration)
...
To learn more about the self-service login flow, read
[this document](https://www.ory.sh/docs/kratos/self-service/flows/user-login)
...
Next, start the application locally and run Ory Tunnel to create a connection between the application and Ory APIs:
...
Finally, go to [http://localhost:3000](http://localhost:3000) in your browser. From there, you can try registering and signing-in
using the Ory Elements-powered pages.
