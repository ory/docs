...
access that clients have to protected resources. Permissions ensure a user has access to _their_ resources only, not the resources
of other users.
...
Because mobile apps are considered _public clients_, it's highly recommended to use the Authorization Code Flow with the PKCE
extension.
