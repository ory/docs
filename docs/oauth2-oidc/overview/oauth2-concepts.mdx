...
access to protected resources. To achieve this, the resource server uses the OAuth2 token introspection endpoint, which is an API
endpoint exposed by the authorization server.

The token introspection endpoint is used to validate the access token itself. It takes an access token as an input and returns
metadata about the token, such as its expiration date, scope, and other information. The resource server can use this information
to determine whether the token is valid or not and whether the requested resource should be served.
