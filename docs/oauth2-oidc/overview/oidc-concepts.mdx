...
OpenID Connect extends the authentication capabilities of OAuth by including components such as an _ID token_ issued as a JSON Web
...
The ID token asserts the identity of the user, specifies the issuing authority, and is generated for a particular audience (that
is, client). It may contain a nonce, specify when and how the user was authenticated, and include additional requested details
about the subject, such as name and email address. The ID token has an issue time and an expiration time, and is digitally signed,
