...
There are three main authentication mechanisms used in the Client Credentials flow:
...
mechanism, the client sends its client ID and client secret as part of the Authorization header in an HTTP request. The
Authorization header is formed by concatenating the urlencoded client ID and client secret, separated by a colon, and base64
encoding the result.
