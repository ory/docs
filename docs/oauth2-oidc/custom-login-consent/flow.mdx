...
### Skipping consent for trusted, first-party clients

For trusted, first-party OAuth2 clients you can configure the client so that the user doesn't have to accept the consent. In the
Ory Network Console, the setting is part of the OAuth2 client edit form.

Consider the `challenge` object returned from
[getOAuth2ConsentRequest](https://www.ory.sh/docs/hydra/reference/api#tag/oAuth2/operation/getOAuth2ConsentRequest). If the
consent is skipped for the OAuth2 client, `challenge.client.skip_consent` is true.

For TypeScript, the following snippet shows how to determine whether to skip the consent UI:
...
// TODO: show the consent UI
// ...
```

A full-application example of a consent screen can be found at
[github.com/ory/hydra-login-consent-node](https://github.com/ory/hydra-login-consent-node/blob/f6ab9d1156aeabbd941f9920ff85408ca0a1f2d4/src/routes/consent.ts#L41).
