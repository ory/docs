...
Let's talk about how OpenID Connect scope and claims work with Ory OAuth2 and OpenID Connect. We'll also cover the default setup
of Ory, which uses Ory Identities for logging in and the Ory Account Experience for getting user consent. This setup supports the
`email` and `profile` OpenID Connect scopes.
...
identity management process to fit your needs. For more information, check out our
