...
### Preserve non-promoted claims in `ext`

When mirroring is disabled, custom claims that are not listed in `allowed_top_level_claims` are dropped from the access token. To
keep these claims in the `ext` object, set `oauth2.preserve_ext_claims` to `true`:

```shell
ory patch oauth2-config --project <project-id> --workspace <workspace-id> \
  --replace "/oauth2/preserve_ext_claims=true" \
  --replace "/oauth2/mirror_top_level_claims=false" \
  --replace "/oauth2/allowed_top_level_claims=[\"some_custom_claim\"]"
```

For example, if you set `some_custom_claim` and `another_claim` during consent, but only `some_custom_claim` is in
`allowed_top_level_claims`, the access token has this structure:
...
Without `preserve_ext_claims`, `another_claim` would not appear in the token.
