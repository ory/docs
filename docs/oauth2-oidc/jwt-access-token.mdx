...
To configure Ory OAuth2 to issue JWT access tokens for all clients, it is necessary to update the configuration as Ory defaults to
opaque tokens:
...
- JSON Web Tokens can't hold secrets: Unless encrypted, JSON Web Tokens can be read by everyone, including third parties.
  **Therefore, they can't keep secrets.**
...
Required JWT claims cannot be overwritten by custom claims.
