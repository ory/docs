---
id: proxy-websockets
title: Define WebSockets rules with Oathkeeper
---

import { useLatestRelease } from '@site/src/hooks'
import CodeBlock from '@theme/CodeBlock'

This guide shows how to use Ory Oathkeeper with
...
Let's create a simple echo WebSocket service that sends back an accepted
message. We'll use the [Gin Web framework](https://github.com/gin-gonic/gin) to
build our application, and [Ory Kratos](https://github.com/ory/kratos) to handle
user login, sign-up, and verification flows.

## Install Ory Kratos and Ory Oathkeeper

You can create any directory for testing and create a `docker-compose.yml` file
...
2. Create a `oathkeeper` folder and `oathkeeper/oathkeeper.yml` with the
   following content:
...
[cookie authenticator](https://www.ory.sh/docs/oathkeeper/pipeline/authn#cookie_session)
against Ory Kratos and proxies only authenticated requests to `http://ws:8080`
upstream. The `ws` hostname is resolved through the Docker network. If you
aren't deploying your application within Docker, this would just be your
localhost IP.

## WebSocket service

1. Let's create a folder `ws` and create our WebSocket service using Go and Gin
   framework. Create `ws/main.go` file with the following content:

```mdx-code-block title="ws/main.go"
import app from '!!raw-loader!../../../code-examples/oathkeeper/main.go'
...
```mdx-code-block title="ws/index.html"
import index from '!!raw-loader!../../../code-examples/oathkeeper/index.html'
<CodeBlock language="html">{index}</CodeBlock>
```

4. Create `ws/Dockerfile` with the following content:

```mdx-code-block title="ws/Dockerfile"
import dockerfile from '!!raw-loader!../../../code-examples/oathkeeper/Dockerfile'
