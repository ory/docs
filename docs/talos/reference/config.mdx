...
Every configuration key can be set via an environment variable. Ory Talos uses the `TALOS_` prefix
and converts dot-separated config paths to uppercase with underscores:
...
| `cache.redis.pool_size`          | integer                   | `100`                | `TALOS_CACHE_REDIS_POOL_SIZE`          | Connection pool size. (restart required, Commercial)                                          |
| `cache.redis.timeout`            | string                    | `3s`                 | `TALOS_CACHE_REDIS_TIMEOUT`            | Redis operation timeout (duration string). (restart required, Commercial)                     |
...
| Key                        | Type    | Default | Env Var                          | Description                                                                                   |
| -------------------------- | ------- | ------- | -------------------------------- | --------------------------------------------------------------------------------------------- |
| `last_used.flush_interval` | string  | `30s`   | `TALOS_LAST_USED_FLUSH_INTERVAL` | Maximum time between batch flushes (Go duration string, e.g. '30s', '1m'). (restart required) |
