...
`cache.type` and the entire `cache.memory.*` block are immutable. Changes require a server restart to take effect.
...
Use the in-memory cache for single-node deployments, or when each instance handles enough traffic to benefit from local caching on
its own. For multi-instance deployments, use [Redis](redis.md) to share a cache across all instances.
