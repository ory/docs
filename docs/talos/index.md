...
Ory Talos exposes two HTTP surfaces. The **admin API** (`/v2alpha1/admin/*`) covers issue, import, rotate, revoke, derive, and
verify. It has no built-in authentication, so put it behind a network boundary or an authenticating proxy. The **public API**
