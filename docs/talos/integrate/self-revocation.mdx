...
Self-revocation lets an API key holder revoke their own key by proving possession of the secret. It is the only operation on the
self-service surface and does not require admin access.

## Prerequisites

You need a running Ory Talos server. See the [quickstart](../quickstart/open-source.mdx) to start one locally.
...
Self-revocation works only for issued and imported API keys. Derived tokens (JWTs and macaroons) are stateless and can't be
revoked.
