...
[google.rpc.Status](https://cloud.google.com/apis/design/errors#error_model) in a top-level `error` object.
...
The HTTP status code follows the canonical [gRPC-to-HTTP mapping](https://cloud.google.com/apis/design/errors#http_mapping). For
...
- `UNAVAILABLE` (HTTP 503) — the server is temporarily overloaded. Retry with exponential backoff. Honor the `Retry-After` header
  when present.
- `DEADLINE_EXCEEDED` (HTTP 504) — the request timed out. Retry with backoff.
- `RESOURCE_EXHAUSTED` (HTTP 429) — the server throttled the request. Honor the `Retry-After` header when present, then retry with
  backoff.
...
- `FAILED_PRECONDITION` (HTTP 409) — the operation didn't meet a precondition. It shares HTTP `409` with `ALREADY_EXISTS`, but
  `error.status` distinguishes the two. Fix the precondition before retrying.
...
([AIP-155](https://google.aip.dev/155)). Use a stable, client-generated UUID per logical issuance. The value is capped at 36
characters.
