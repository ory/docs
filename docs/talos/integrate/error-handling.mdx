...
- **`UNAVAILABLE` (HTTP 503)** — the server is temporarily overloaded. Retry with exponential backoff. Honor the `Retry-After`
  header when present.
- **`DEADLINE_EXCEEDED` (HTTP 504)** — the request timed out. Retry with backoff.
- **`RESOURCE_EXHAUSTED` (HTTP 429)** — the server throttled the request. Honor the `Retry-After` header when present, then retry
  with backoff.
