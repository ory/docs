...
An issued API key is a credential that Ory Talos generates. The admin surface creates the key, returns the full secret once at
creation, and stores only a hash afterward. Use issued keys when you want Ory Talos to own the full lifecycle — generation,
lookup, rotation, and revocation — for the credentials your customers or services use to call your API. For how issued keys
compare to imported keys and derived tokens, see [credential types](../concepts/credential-types.md#issued-api-keys).

This guide walks through the issue-and-verify flow end to end. CI runs every example.

## Prerequisites

You need a running Ory Talos server and the `talos` CLI. See the [quickstart](../quickstart/open-source.mdx) to start one locally.
...
- **`secret`** — The full API key credential. Talos returns this value **only once** and never exposes it again. Store it
  securely.
...
For the complete field reference, see the [VerifyAPIKey API reference](../reference/api/admin-verify-api-key.api.mdx).
...
[AIP-160](https://google.aip.dev/160) `filter` expression on the indexed fields `actor_id` and `status`, for example
