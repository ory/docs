...
- **Leaf grant on a file.** The rule covers only the original target inode. The renewed target is a fresh inode that is not in
  the allowlist, and the kernel denies reads through the symlink with `EPERM`. Restart `kratos serve` after the swap so the rules
...
To make cert renewals robust, add the cert directory (for example `/etc/letsencrypt`) to `security.landlock.allowed_paths`
instead of relying on the per-file grants Kratos derives from the TLS path configuration.
