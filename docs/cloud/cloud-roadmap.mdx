---
id: cloud-roadmap
title: Ory Cloud Roadmap
sidebar_label: Roadmap
---

This document outlines product development plans for Ory Cloud for the next
quarters. Please note that the roadmap is subject to change. The product
development team is actively working on the roadmap and adjusting priorities to
deliver the best value to Ory users.

## Roadmap

### Q3 2021

- [x] Enriching Ory Cloud Console capabilities
  - Detailed view for identities
  - SDK URLs for project overview
- [x] Configuration for redirection flows
  - Post login and registration redirection
  - Post settings,verification and recovery redirection

### Q4 2021

- [x] Multi-Factor Authentication (MFA)
  - Self-service in Ory Cloud Console
- Extending identity management self-service capabilities
  - Updating and deleting identities
  - Recovering an identity with email code
- [x] Bring your own UI
  - Self-service in Ory Cloud Console
- [x] Extending documentation
  - Documenting all public APIs
  - Refactoring examples with support for multiple programming languages

### Q1 2022

- [x] Ory Project membership
  - Inviting users with a confirmation
  - Removing members of Ory Project
  - Assigning roles to members
  - Updating member roles
- Self-service for integration with Social Identity Providers
  - Sign in with Google
  - Sign in with Github
  - Sign in with Facebook
- Session management
  - Display active identity sessions in the project
  - Revoke identity session
- Extended Ory Project management capabilities
  - Administrative dashboard
  - Deactivating project
  - Deleting project

## Feedback

We highly value feedback from Ory Cloud users and open source adopters. If you
have feedback you would like to share, or you haven't found a certain feature
please use [GitHub Discussions](https://github.com/ory/cloud/discussions) or
directly contact us on the [Ory Slack](https://slack.ory.sh/).
