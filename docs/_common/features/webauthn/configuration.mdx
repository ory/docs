...
Once `passwordless` is set to either `true` or `false`, avoid changing it. Doing
...
<Tabs>
  ...
  --replace '/selfservice/flows/methods/webauthn/enabled=true' \\
  --replace '/selfservice/flows/methods/webauthn/config/passwordless=${props.passwordless || false}' \\
  --replace '/selfservice/flows/methods/webauthn/config/rp/display_name="My Display Name"'`}</CodeBlock>
  </TabItem>
