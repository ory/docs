---
id: custom-domains
title: Custom domain and CORS setup
---

To use Ory Cloud on one or more domains that you own, you can set a CNAME record.
...
The domain you enter in the first step will be the domain that you'll use for requests to Ory Cloud. You will have to create a
...
[Cookie configuration section in the Kratos docs](https://www.ory.sh/kratos/docs/guides/configuring-cookies/) and
...
`auth.foobar.ory` set as our custom domain which will point to your Ory Cloud Project. It is important that you add CORS origins
to your custom domain to **ensure Single Page Applications (SPAs) support**. We allow up to **50 origins**. If you plan on us ing
...
When you've added the domain in Cloud Console, we will continuously recheck its status to see if it was configured with your
...
Once active the Ory Managed UI is reachable at `https://<your-custom-domain>/ui/welcome`. Learn how to configure a custom UI:
...
3. The value, which is provided by us. Check the value provided in the form (detailed in the first step of this article) to know
   what this should be for your Ory Cloud project.
