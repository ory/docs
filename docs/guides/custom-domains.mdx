...
To prevent this, configure a custom SMTP server, that matches at least the top-level domain (TLD) of your custom domain. For
example, if you configured the custom domain `ory.your-custom-domain.com`, you should also configure an email server that lives
under `your-custom-domain.com`.
