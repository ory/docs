...
However, when using a custom domain in the Ory Network without a custom SMTP server, links inside the automated Ory Identities emails
...
To prevent this configure a custom SMTP server, that matches at least the top-level domain (TLD) of your
