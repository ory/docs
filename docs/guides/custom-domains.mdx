...
We do not allow setting the CORS origins to be `*` or `null`. You can, however,
use a wildcard subdomain such as `https://*.foobar.ory`. To be on the safe side,
we recommend setting the CORS origins to be an exact match instead of a wildcard
subdomain. :::
