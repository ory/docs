...
Now let's create a GitHub Action that will update the configuration when a pull request is merged.
...
In order for the GitHub Action to authenticate with Ory Network, you need to add the ORY_USERNAME and ORY_PASSWORD secrets to your
...
Actions webhooks. You can save and version these snippets in git. There are many options for managing Jsonnet snippets in git.
...
When you make changes, convert the new OIDC mapping or Ory Actions payload to base64 and add it to the config file.
