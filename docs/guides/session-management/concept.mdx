---
id: concept
title: The Ory Session
---

When an identity or end-user authenticates (for example signs in using their
email and poassword), they receive an Ory Session. The Ory Session can either be
issued as an

1. **Ory Session Cookie**: When Ory detects a browser, it stores a cookie in
   that user's browser. This cookie represents the user's session.
2. **Ory Session Token**: When Ory detects a non-browser client (for example a
   native mobile app), it issues an Ory Session Token to that client.
