...
:::tip To get the Session ID, call the `/sessions/whoami` endpoint or
`toSession`. :::
...
refreshed by adjusting the `earliest_refresh` configuration.

For example, if you set `earliest_refresh` to `24h`, sessions can't be refreshed
...
`earliest_refresh` to `lifespan`, which allows sessions to be refreshed during
