...
to allow session refresh only for a specific time before the session expires. If
we set the `earliest_refresh` equal to `lifespan`, it's possible to refresh even
new sessions.
