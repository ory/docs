...
the current session. Use this feature to show a UI with all active sessions to
the user.
...
A user can also revoke their other sessions using the
...
This essentially logs the user out of all other sessions on any device. Through
self-service revoked sessions aren't deleted. Instead, they become inactive.
Inactive sessions are retrievable and deletable through the administrative API.
...
Retrieve all sessions for a specific identity using the administrative
...
to delete all sessions for a specific identity is:
