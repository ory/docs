...
To check if the user is signed in, call the `/sessions/whoami` endpoint. When the user is doesn't have
a session, you get a `401 Unauthorized` response. If the user has a session, you get a `200 OK` response 
...
the Ory Kratos Session Token as the Bearer Token in the HTTP `Authorization` header:  
