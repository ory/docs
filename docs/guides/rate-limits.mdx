...
1. **Project rate limits:** Based on your subscription plan and environment (Production, Staging, or Development). These control
...
violations for monitoring purposes but don't block requests, while enforced endpoints return HTTP 429 when rate limits are
