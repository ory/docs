...
1. [**Project Rate Limits in Workspaces**](#project-rate-limits-in-workspaces): These limits are determined by your Ory subscription plan and project environment.
   ...
   2. Project Rate Limits are defined in terms of: <br/>**Burst Limit:** This governs the maximum number of requests allowed
...
For a detailed explanation of Workspaces and Environments, please refer to our [Workspaces and Environments guide](link-to-workspaces.mdx).
...
Note: Development environment in Production plans uses the same rate limits as the Staging environment.
...
Note: Development environment in Growth plans uses the same rate limits as the Staging environment.

### Important Notes

- **Individual Project Limits**: Rate limits are applied individually to each project based on its assigned environment (Production, Staging, or Development) and your Workspace subscription plan. Projects do not share a combined limit.
- **Environment-Specific Limits**: Each environment (Production, Staging, Development) within a project has its own set of rate limits as defined in the tables above.
- **Subscription Plan Impact**: Your Workspace subscription plan determines the available rate limits for each environment type across all projects in the Workspace.
- **Enterprise Solutions**: For higher rate limits or custom configurations, please [contact Ory](https://www.ory.sh/contact/) to discuss enterprise-grade solutions.
...
Endpoint-Based Rate Limits are precise controls applied to individual API endpoints within your Ory projects. Unlike Project Rate Limits,
which govern overall project request volumes, Endpoint-Based Rate Limits focus on safeguarding specific functionalities against abuse.
