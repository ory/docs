...
Ory allows you to import identities from any other system. The endpoint used is
the same as when [creating identities](./managing-users-identities.mdx) with the
subtle difference that you also provide the `credentials` field.
...
over to the [social sign in documentation](../social-signin/overview).
