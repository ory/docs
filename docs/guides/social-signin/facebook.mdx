...
7. You should now be on the dashboard. In the "Facebook Login" card, click **Set
Up**. Disregard the quickstart - instead, in the sidebar, click Settings >> **Basic**.
...
Facebook only returns `access_token` and doesn't return `id_token`. Ory
sends requests to
