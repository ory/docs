...
To add Facebook as a social sign-in provider, you need a Facebook Developer
account. Go to
...
Follow these steps to add Facebook as a social sign-in provider for your for
your Ory Cloud project using the Ory Cloud Console.
...
7. In the "Facebook Login" card, click **Set Up**. Ignore the quickstart and
   select **Settings → Basic** from the sidebar.
8. Copy the Client ID and Client secret from Facebook and paste them into the
   corresponding fields in the Ory Cloud Console.
9. On Facebook Developers, click **Save Changes**.
10. From the left navigation bar, select **Facebook Login → Settings**.
11. Paste the redirect URI copied from Ory into the **Valid OAuth redirect
    URIs** and click **Save Changes**.
12. In the Ory Cloud Console, click **Save Configuration** to enable Facebook as
    a social sign-in provider.
...
[Facebook](https://developers.facebook.com/docs/facebook-login/) only returns
`access_token` and doesn't return `id_token`. Ory sends requests to
...
Follow these steps to add Facebook as a social sign-in provider to your Ory
...
Follow these steps to add Google as a social sign-in provider to your when
