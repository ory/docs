...
Follow these steps to add Discord as a social sign-in provider to your Ory Cloud project
using the Ory CLI:
...
3. Create a Jsonnet code snippet to map the desired claims to the Ory Identity schema.
  ...
  Discord doesn't implement OpenID Connect. Ory makes a request
  to
  [Discord's User API](https://discord.com/developers/docs/resources/user#get-current-user)
  and adds the data to `std.extVar('claims')`. Not all fields are
