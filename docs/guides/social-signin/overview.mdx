...
to social sign-in provider accounts after email verification. To disable this behavior, 
add the `session` hook after one of the registration methods (for example `password`):
...
While convenient for users, linking accounts creates an attack vector that can
