...
You can get the OIDC / OAuth 2.0 access, refresh, and ID tokens issued for the
Identity by social sign-in providers.
...
Run this command to get the Identity details that include the social sign-in
provider tokens:
...
When the call is successful, the system returns the Identity details with the
available social sign-in provider tokens:
...
By default, tokens are stored as plain text recorded with the default Noop
cipher.

To increase the security, you can choose to encode the tokens with AES or
XChaCha20 Poly1305 ciphers. Read
[this document](../../kratos/guides/select-cipher-algorithm) to learn how to
choose the cipher.
