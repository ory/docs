...
Follow these steps to add NetID as a social sign-in provider to your Ory Cloud project
using the Ory CLI:

1. Create a [NetID Service](https://developerzone.netid.dev/devportal/tutorial/services/) and [NetID Client](https://developerzone.netid.dev/devportal/tutorial/clients/).
...
3. Create a Jsonnet code snippet to map the desired claims to the Ory Identity schema.
  ...
  [NetID](https://developerzone.netid.dev/sso/)
  returns only the `sub` and `sub_legacy` claims in the `id_token`.
  Ory makes a request to
  [NetID's /userinfo API](https://developerzone.netid.dev/sso/#userinfo-endpoint) and adds the
...
4. Encode the Jsonnet snippet with [Base64](https://www.base64encode.org/) or host it under an URL accessible 
to Ory Cloud.
5. Download the Identity Service config from your Ory Cloud project and save it to a file: 
  ...
  ```
<a name="config"></a>
6. Add the social sign-in provider configuration to the downloaded config. Add the Jsonnet snippet
...
5. Add the social [sign-in provider configuration to the Ory Kratos configuration](#config). Add the Jsonnet snippet
