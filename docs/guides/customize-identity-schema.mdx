...
Ory supports custom Identity Schemas. The Identity Model is a JSON Schema which
describes the traits that make up an identity, for example customers, users or
staff accounts.
...
It is possible to add metadata to an identity, either as protected field that
can only be read by the admin only or as public field that can be read by the
user as well.
