...
From experience, we at Ory have noticed many resources online create confusion on how to
add Authentication to applications. This is even worse on frameworks that
allow support for so many devices as it becomes difficult to understand where to
use session cookies, tokens or even OAuth2 flows.
...
Start the Ory Cli proxy on [http://localhost:4000](http://localhost:4000) and set the proxied application to [http://localhost:3000](http://localhost:3000).
