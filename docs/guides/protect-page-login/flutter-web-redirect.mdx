...
http://localhost:4000/.ory) and the path to the Ory API `/api/kratos/public`.
...
Start the Ory Cli proxy on [http://localhost:4005](http://localhost:4005) and
set the proxied application to [http://localhost:3005](http://localhost:3005).

```shell-session
ory proxy --port 4005 http://localhost:3005
```
