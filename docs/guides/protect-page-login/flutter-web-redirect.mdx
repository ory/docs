...
package and for managing environment variable we will use [flutter dotenv](https://github.com/java-james/flutter_dotenv).
...
Next, create an Authentication service in our `lib/services` directory,
...
Create an `.env` file in the root of our project to hold the
`ORY_BASE_URL` variable. This would be the Ory proxy URL (for example
...
session check doesn't detect a valid session and redirects to the login page of
