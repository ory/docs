...
[flutter dotenv](https://github.com/java-james/flutter_dotenv) for environment
variable management.
...
Next, create an Authentication service in the `lib/services` directory. This
...
Create a `.env` file in the root of the project to hold the `ORY_BASE_URL`
...
Finally, update the `lib/main.dart` file to check for a session cookie on the
...
[Flutter Web applications in production](https://docs.flutter.dev/deployment/web).
