...
any browser redirects. This will be discussed in a separate guide.

For HTTP requests we will use the [dio](https://github.com/flutterchina/dio) package.
...
Then we need to create an `.env` file in the root of our project to hold the `ORY_BASE_URL` variable. This would be the Ory proxy URL (e.g. http://localhost:4000/.ory) and the path to the Ory API `/api/kratos/public`.
...
initial load of the application. This will determine if we redirect to the sign-in/signup page or not.
...
Start the Ory Cli proxy on [http://localhost:4005](http://localhost:4000) and
set the proxied application to [http://localhost:3000](http://localhost:3000).

```shell-session
ory proxy --port 3004 http://localhost:3000
```
