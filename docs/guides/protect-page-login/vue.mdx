...
Ory APIs are now mirrored on `http://localhost:4000`. Use that URL as the `baseUrl` for the `@ory/client` SDK (see `HelloWorld.vue`
code above). The `--dev` flag disables security checks for easier integration and should not be used when deploying the Tunnel
...
Access your single-page app at [localhost:8080](http://localhost:8080/), sign in, and see if you can make authenticated
