...
Next we add a session check to the default home page of the React example
application.The highlighted code is what we added to check whether the user is
signed in, get the logout url flow and redirect them to the login page if not.
There is also a welcome message with the registered user email and a logout
link:
...
`baseUrl` for the `@ory/client` SDK (see code above). The `--dev` flag disables
security checks for easier integration and should not be used when deploying the
Tunnel to a staging environment.
...
Then open [http://localhost:3000](http://localhost:3000) in your browser. After
loading, you now are presented with Ory's Sign In page! Let's click on sign up
