...
available on the hostname `localhost` as well (for example `http://localhost:3001`).
That's why we need the Ory CLI, because it has a proxy included which mirrors
...
login. At this point we'ven't set up any custom UI management and thus will
...
domain (such as `https://ory.example.com` and `https://www.example.com`).

The easiest way to connect Ory to your domain is to connect Ory to a subdomain
of yours. You can do this by
[adding a Custom Domain to your Cloud project](../custom-domains.mdx)!

With the custom domain set up, you don't need the Ory Proxy anymore and will
