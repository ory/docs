...
The easiest way to achieve that is by using the [Ory Tunnel]. Ory Tunnel is a tool that is a part of the 
...
Running this commands opens the `4000` port on your local machine. This address is connected to your Ory Cloud project and its APIs. 
The value passed in the `--dev` flag must point to the host and port of the application you develop locally. In the example, the
...
Your local machine is usually available on both `localhost` and `127.0.0.1`. For the browser these two domains are
