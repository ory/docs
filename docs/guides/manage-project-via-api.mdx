...
You can configure Ory Network projects using the [Ory Console](https://console.orystg.com/), as well as the
...
[overview](https://console.orystg.com/workspaces) or when you click into a workspace as part of the URL.
...
```bash
curl --location --request GET "https://api.console.orystg.com/workspaces/${WORKSPACE_ID}/projects" \
...
```bash
curl --location --request POST 'https://api.console.orystg.com/projects' \
...
curl --location --request DELETE "https://api.console.orystg.com/projects/${ORY_PROJECT_ID}" \
  --header "Accept: application/json" \
  --header "Authorization: Bearer ${WORKSPACE_API_KEY}"
```
...
curl --location --request GET "https://api.console.orystg.com/projects/${ORY_PROJECT_ID}" \
  --header "Accept: application/json" \
  --header "Authorization: Bearer ${WORKSPACE_API_KEY}" | jq
```
...
curl --location --request PATCH "https://api.console.orystg.com/projects/${ORY_PROJECT_ID}" \
...
```bash
curl --location --request GET  "https://api.console.orystg.com/projects/${ORY_PROJECT_ID}/tokens" \
...
```bash
curl --location --request POST "https://api.console.orystg.com/projects/${ORY_PROJECT_ID}/tokens" \
-d '{"name":"example"}' \
-H "Content-Type: application/json" \
-H "Authorization: Bearer ${WORKSPACE_API_KEY}" | jq
```
...
curl --location --request DELETE "https://api.console.orystg.com/projects/${ORY_PROJECT_ID}/tokens/${ORY_PROJECT_API_KEY_ID}" \
-H "Accept: application/json" \
-H "Authorization: Bearer ${WORKSPACE_API_KEY}"
```
