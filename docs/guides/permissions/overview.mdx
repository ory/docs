...
The data model used by Ory Permissions are so-called [relationships](/keto/concepts/relation-tuples.mdx) that encode relations
...
The following relationships showcase the namespace configuration. In short, it sets up a `developer` group with two members, and a
