...
an [indirection](https://en.wikipedia.org/wiki/Indirection) (e.g. all members of Group:group1). The object is referenced by its ID
(e.g. Group:group1, File:file1).
...
As a very simple example, leat's assume the following tuples exist:
...
- Is `user2` a member of `group1`?
  ...
  ```keto-natural
  is users:user2 in members of Group:group1?  // No
  ```
