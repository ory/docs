...
The Ory Permissions ([Keto](https://github.com/ory/keto)) implements Google Zanzibar. It might differ a bit from other
authorization services you know, so let's first clarify a few concepts.

:::tip

Ory Permissions is available to all Ory Network users without additional fees or setup. The API is in a production-ready
...
The data model used by Ory Permissions are so-called [relation tuples](/keto/concepts/relation-tuples.mdx) that encode
