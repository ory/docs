...
Save the file as `tuples.json` in your current working directory. To create these relationships in Ory Permissions, run this
command:
...
With the permissions created, try running queries that illustrate use cases:
...
- Learn [what is a permission model and how to create one](../../keto/modeling/create-permission-model.mdx).
