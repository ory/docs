...
If you haven't already, sign up at Ory Cloud Console:
...
Your Ory Cloud account isn't limited to a single project. You can create as many
as you like, and each project can have its own subscription.
...
Note that a personal access token is required to access administrative APIs (for
example deleting an identity, using the Ory CLI, ...). Personal Access Tokens
are bound to a project.
...
[follow the next steps](../index.mdx)!
