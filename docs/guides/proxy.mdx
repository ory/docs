...
Using Ory Cloud without Ory Proxy is not encouraged, unless you are
writing a mobile application or are only interacting with Ory Cloud's Admin
APIs.

If you haven't already, please [install the Ory CLI](./ory-cli-install-use.mdx)
...
Ory Proxy has shorthand URLs which you can use to initiate self-service
flows:
...
Ory Proxy translates any known Ory credentials (for example an Ory Session Token or Ory
...
crashes, Docker won't be notified. To get a more stable system in place,
please follow
