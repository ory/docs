...
When implementing MFA in your application, you might want to prompt signed-in users
to provide their configured second factor. 

To do that, initiate a new login flow using one of these endpoints and setting the
`aal` parameter to `aal2`:
