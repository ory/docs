...
to allow users to perform 2FA with with TOTP apps such as the Google Authenticator. 
...
Follow this steps to enable Time-Based One-Time Password (TOTP) as the second
...
`totp` method to `selfservice/methods` in the configuration file: 
