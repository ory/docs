...
You can enable Time-Based One-Time Password authentication in the Ory Identity
Service (Kratos) to allow users to perform 2FA with TOTP apps such as the Google
Authenticator.
...
When attempting to perform actions secured with 2FA, users get a TOTP code with
...
3. Define the name of the requesting party in the **Display Name**. The users
   sees this name in the TOTP application when getting a one-time password.
...
add the `totp` method to `selfservice/methods` in the configuration file:
...
To help users distinguish which identity requests for the TOTP code, add a
`totp` object to the trait which represents the TOTP account name.
