...
All Ory components use the same configuration format and APIs whether they are
...
This feature is currently in development. More configuration options are added
with new CLI releases
...
[Ory Kratos configuration format](../../kratos/reference/configuration.md)
except for some keys (for example `serve`, `dsn`) which are ignored.
...
When patching configuration, the part after `=` is interpreted as raw JSON. Use
these format to patch the desired data types:
