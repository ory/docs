...
The CLI contains two important tools for local development and self-hosted instances
of the Ory Identity Service (Kratos): 
...
Using the Proxy is an alternative to custom domains (CNAME), and a useful tool when developing locally. 
Even though you can use the Proxy in production, we recommend using our Custom Domain feature where possible.

:::

When an application runs on `http://localhost:3000`, Ory must be available 
on the same domain, for example at `http://localhost:3001`. Thanks to such setup, 
...
"upstream" (the URL protected by the Ory Proxy). 
...
You can validate and decode the JWT using the public key.
...
The Ory Tunnel has the same funtionality and behaves the same way 
as the Proxy with these exceptions:  
  ...
  and does't use JWTs natively <br />
  ❌ Add the `.ory` prefix to any of the Ory URLs
...
`https://<proxy-host>/.ory/ui/login` is the same as calling `https://<your-project-slug>.projects.oryapis.com/ui/login`
directly.

### Self-service Flows

Ory Proxy and Tunnel provide URLs which you can use to initiate self-service flows:
...
<Tabs groupId="proxy-tunnel">
  <TabItem value="proxy" label="Ory Proxy">
  ...
  <TabItem value="tunnel" label="Ory Tunnel">

  | URL   |  Self-service Flow  |
  |:--- |:--- |
  | `/self-service/login/browser`| [Login](/kratos/self-service/flows/user-login)  |
  | `/self-service/registration/browser`| [Registration](/kratos/self-service/flows/user-registration) |
  | `/self-service/recovery/browser`| [Recovery](/kratos/self-service/flows/account-recovery)  |
  | `/self-service/verification/browser`| [Verification](/kratos/self-service/flows/verify-email-account-activation) |
  | `/self-service/settings/browser`| [Settings](/kratos/self-service/flows/user-settings) |
...
You can also append a `return_to` query parameter to the URL. This redirects the user
...
```html
<a href="/self-service/login/browser?return_to=https://localhost:4000/my-url">Log in</a>
```
