...
:::warning DEPRECATED Ory Proxy has been deprecated and is no longer recommended for new implementations. Please use Ory Tunnel
instead, which provides similar functionality with an improved architecture. This documentation is kept for reference purposes for
users who still have Ory Proxy in their existing implementations. :::
