...
```mdx-code-block
<details>
<summary>Expand to learn how to configure webhooks in your Ory Network project.</summary>
<div>
```
...
2. Add the webhook to your project configuration file:

```yaml title="project-configuration.yaml"
session:
  lifespan: # ...
# ...
# highlight-start
...
3. Update your project's configuration using the file you worked with:
...
- API key authentication. Type must be set to `api_key`. All properties are mandatory.
  ...
  Read [Authorization with API Keys](../concepts/personal-access-token.mdx) to learn how to create and use API Keys.
