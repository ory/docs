...
Ory Actions support webhooks, which are HTTP callbacks that can be triggered by specific events in your Ory Actions-powered
application. Webhooks allow Ory Actions to notify external systems, such as Hubspot, Mailchimp, and others, when certain events
occur, such as a user registration or profile update.

When to use webhooks:

1. Integration with third-party systems: If you want to integrate your Ory Actions-powered application with another system, such
   as a CRM, email marketing platform, or analytics tool, webhooks can be used to trigger updates in that system when events occur
   in your Ory Actions application.
2. Automated workflows: You can use webhooks to automate workflows and processes that span multiple systems. For example, you
   could use webhooks to automatically add a new lead to your CRM when a user registers in your Ory Actions application.
3. User data modification: Webhooks can be used to modify user data when certain events occur. For example, you could use webhooks
   to update user data in your Ory Actions application when a user updates their profile information in another system.

:::note

When using webhooks, it is important to ensure that the data being transmitted is secure and that the external system you are
...
```mdx-code-block
<details>
<summary>Expand me to learn how to configure webhooks in your project.</summary>
<div>
```

While Ory does not yet support webhook configuration in the UI, we provide examples in YAML format which you can copy and paste
into your project configuration file. We use the Ory CLI to get the project configuration file:
...
Then, you add the webhook to your project configuration file:
  ...
  flows:
    registration: # or login, recovery, verification, settings
      after:
        ...
        oidc:
          after:
            # Actions in this sections will be executed after sign up (or login, settings) with a social sign in provider / OpenID Connect
            # ...
# highlight-end
```
...
ory update identity-config {project.id} \
  ...
  --format yaml > project-configuration-updated.yaml
```

And verify that the updated project configuration file is correct.
...
Webhooks trigger HTTP requests to the webhook URL. The request method, URL, authorization, and body can be configured.
...
```mdx-code-block
<details>
  <summary>Expand me to see an example of all properties of the <code>ctx</code> object for a registration flow.</summary>
  <div>
```
...
`identity.public_metadata` or `identity.admin_metadata` in it, the values from that object will be used to replace the
[identity metadata](../kratos/manage-identities/10_managing-users-identities-metadata.mdx) fields before they are saved to the
database.

:::info

This method replaces metadata object. It is not possible to update only a single value in `metadata_public` or `metadata_admin`
respectively.
