...
**Note:** All other parameters—such as pod labels, service accounts, and resource configurations—are inherited from your
production Hydra deployment. This minimizes potential configuration discrepancies and reduces the risk of errors.
...
In case you want to revert all the database changes introduced by the last migration, check logs of the migration job looking for
a similar line:
...
helm template hydra ory/hydra --namespace oasis --show-only templates/job-migrations-custom.yaml \
 ...
 -f hydra/oel/values-migration-down.yaml > migrate-down-job.yaml
```
