...
You can configure the mapping of SAML attributes to Ory's identity schema using Jsonnet. All custom SAML attributes defined at
the Identity Provider (IdP) will be available in the `raw_claims` object within claims. These attributes can then be mapped to
...
available in `claims.email`, everythign else is nested under `claims.raw_claims`.
