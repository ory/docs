...
### Onboarding portal (preview)

To enable organization admins to set up SAML SSO connections for their organization, you can create a self-service link for them.
...
The generated link has an expiry date. Once the link expires, it will no longer work. Additionally, you can revoke the link and
generate a new one.
...
#### Revoke an onboarding portal link

```shell
curl -X DELETE --location "https://api.console.ory.sh/projects/$PROJECT_ID/organizations/$ORGANIZATION_ID/onboarding-portal-links" \
     -H "Authorization: Bearer $WORKSPACE_API_KEY"
```
