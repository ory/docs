...
On Ory Network and Ory Enterprise License (OEL) deployments, the courier template directory is automatically
allow-listed by the [Landlock filesystem sandbox](../../security-compliance/landlock-sandbox.mdx). Templates are
loaded lazily at send time, so an out-of-tree template path that is not allow-listed will fail with `EPERM`.
