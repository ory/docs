...
2. Importing existing users - when migrating from another system to Ory
   Kratos.
...
use to set up his or her credentials (for example connect to a Social Sign In Provider,
...
It's not possible to send the recovery link directly to a user's
