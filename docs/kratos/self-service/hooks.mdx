...
flow, `hook_2` isn't run for the `password` method, but for the `oidc` method. If the `password` method is used, then `hook_3` will run.
...
method. Support for Web-Hooks before execution of a method is supported only for registration and login methods.
...
`ctx` object. To create a body looking like `{ user_id: <some-id> }` to be
sent to the web hook endpoint, the Jsonnet template would look like this:
...
When performing a registration flow with an API client (such as a mobile app), this
...
executed because the HTTP reply can't be modified further (for example HTTP Status
Code was already sent as 200 and can't be changed to 301). You must ensure that
