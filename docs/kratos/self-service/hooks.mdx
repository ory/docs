...
flow, `hook_2` isn't run for the `password` method, but for the `oidc` method.
If the `password` method is used, then `hook_3` will run.
...
method. Support for Web-Hooks before execution of a method is supported only for
registration and login methods.

:::note

As of today Web-Hooks can't control the execution of the flow by
...
- `body` - URI of a jsonnet template, used by the web-hook to render the payload
  ...
  local files. This property is ignored for HTTP `method`s, which don't support
...
`ctx` object. To create a body looking like `{ user_id: <some-id> }` to be sent
...
This can come in handy in testing. For example, if you cancel the webhook, you
prevent the test accounts created during testing from being added to CRM.
...
Currently, JsonNet doesn't support regular expressions. Follow this issue to see
...
Sometimes you need to notify a system without needing its response. For example,
when a user signs up, their email address is added to the newsletter. If the
system's response is not important, you can ignore the response and make the
webhook execution non-blocking.
...
When performing a registration flow with an API client (such as a mobile app),
this hook creates a session and returns the session token and the session itself
in the response body as application/json:
...
executed because the HTTP reply can't be modified further (for example HTTP
Status Code was already sent as 200 and can't be changed to 301). You must
ensure that the session hook is the last hook in your configuration!
