...
#### Non-Blocking Web Hooks

Sometimes you need to notify a system without needing its response. For example,
when a user signs up and you want their email address to be added to the
newsletter you might not care about the response and favor the execution to be
non-blocking.

For those cases you can set `response.ignore` to `true` in your web-hook config:
