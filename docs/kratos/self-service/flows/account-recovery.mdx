---
id: account-recovery
title: "Legacy: Account recovery and password reset"
---
...
This page describes the outdated, but still supported `link` strategy. We recommend using the new `code` strategy, for the
following reasons:

- Email virus-scanners sometimes open magic links to scan them. This invalidates the link and a user may not be able to revover
  their account.
- API flows initialized by apps on mobile phones or smart devices are not possible when using magic links.

Please read more [here](../../account-recovery-password-reset.mdx).
