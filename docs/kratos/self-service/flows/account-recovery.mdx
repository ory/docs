...
Account recovery can be configured in the "Customize/Account Recovery" screen of
Ory Cloud.
...
<p>
  <figure>
    <img
      ...
      src={useBaseUrl('images/kratos/email-recovery-unknown.png')}
    />
    <figcaption>
...
<p>
  <figure>
    <img
      ...
      src={useBaseUrl('images/kratos/email-recovery-known.png')}
    />
    <figcaption>
...
for verification purposes and the account hasn't yet been activated (see
[Account Activation](verify-email-account-activation.mdx) - if the email address
isn't yet verified, the verification status of the corresponding object in
Kratos will be set to verified upon successful flow finalization.
...
To specify that an identity's trait's a recovery email, use the following
Identity Schema:
...
- `choose_method` indicates that the user hasn't chosen a recovery method yet.
  This is useful when `link` isn't the only recovery method active.
...
The Recovery Flow for API clients doesn't use HTTP Redirects and can be
...
If the recovery challenge (for example the link in the recovery email) is
invalid or expired, the user will be HTTP 303 redirected to the Recovery UI.
