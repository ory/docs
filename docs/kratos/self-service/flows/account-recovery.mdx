...
There are two Recovery Flow types supported in Ory Identities:
...
Account recovery can be configured in the "Customize/Account Recovery" screen of the Ory Console.
...
You can configure the privileged session duration in the Ory Identities config:
...
The Recovery Flow for browser clients relies on HTTP redirects between Ory Identities, your Recovery UI, and the end-user's
browser:
...
The Ory Network offers a default UI implementation. Pointing to another UI endpoint will be supported in the future.
...
When an invalid or expired challenge is used, Ory Identities initializes a new Account Recovery flow automatically. This flow will
...
Completing account recovery always results in a HTTP 303 redirect with a Ory Session Cookie to the Settings UI with a Settings
Flow prompting the user to update their password or credentials:
