...
[Account Activation](verify-email-account-activation.mdx) - if the
email address isn't yet verified, the verification status of the corresponding
...
If the recovery challenge (for example the link in the recovery email) is invalid or
