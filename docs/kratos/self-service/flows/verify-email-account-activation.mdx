...
automatically. A user can also verify their address manually.

The verification flow is supported in both browsers and API clients.

:::caution

Email verification should not be taken as a guarantee that the person currently using the account, is the person that verified the
address. It is recommended to implement additional security mechanisms.
...
Ory Identities supports only allowing users with a verified address to login.
...
When using one-time codes to verify a user's address, Ory Identities sends out an email containing a 6-digit code. The user must
...
When using the "magic link" method, Ory Identities sends the user an email with a "magic link". If the user has access to the
email associated with the and clicks the link, Ory Identities marks that address as verified.
...
- Some email virus scanners open links in emails to scan them. This invalidates the link and may prevent users from recovering
  their account even if they have the access to the defined recovery address.
- Verification flows initialized by apps on mobile phones or smart devices don't work with links.
