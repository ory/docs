...
import CodeLinkComparison from "./partials/code-link-comparison.mdx"
```
...
Completing account verification does not guarantee that the account is used by the person who performed the verification. We
...
field. Now, the user only has to submit the form to complete the verification.
...
account again to receive a new verification email.
