...
Social Sign Up is not possible for API Clients. It will be possible in
...
If the registration payload is invalid (for example the password doesn't match the
...
the user refuses to accept a certain scope (for example "Allow Ory Kratos access to my
website") or if the provider simply doesn't have this data (for example the upstream
...
HTML. Instead, you need to implement the HTML code in your application (for example
