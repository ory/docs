...
[Ory Console](https://console.ory.sh/projects/current/recovery).
...
Ory Identities supports recovery flows in native applications. This allows you to build a native application that allows the user reset their password in the app without opening a browser.
However, this flow is only available for the `code` method.

Since enabling this alters the behavior of the native recovery flow, you need to enable it in the Ory Console or using the Ory CLI. The setting only affects the recovery flow for api clients. The other self-service flows and browser recovery flows are not affected.

For new projects this setting is enabled by default.
...
Go to **Project settings** → **Advanced** and enable **Native Recovery Flows** in the [Ory Console](https://console.ory.sh/projects/current/recovery).
