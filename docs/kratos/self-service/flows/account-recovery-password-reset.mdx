...
```suggestion
If a user has multiple recovery addresses and the `choose_recovery_address` feature flag is [enabled](../../../identities/get-started/account-recovery.mdx#enable-the-feature-flag-choose_recovery_address-unlocks-sending-a-recovery-code-via-sms), the process is as follows:
...
Completing a recovery flow will also verify the identities address if the address is used for both recovery and verification and
the identity is not yet verified when the recovery flow begins.
