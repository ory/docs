...
If you support phone numbers in your identity schema, be sure to configure a
[SMS HTTP gateway](../../emails-sms/10_sending-sms.mdx) so that SMSes are indeed sent.
...
Note that for costs reason, this concept does not exist for SMSes. That means that trying to recover an account with the wrong
phone number will _not_ result in any sent SMS.
...
example if they no longer have access to the original address. To use two (or more) recovery addresses, you need to define a
secondary field, such as `email_secondary` in the traits section of the identity schema. This field will serve as the fallback
address for account recovery. Read more about identity schemas [here](../../manage-identities/05_identity-schema.mdx).
...
Note that this feature is orthogonal to the situation where a user has multiple recovery addresses registered of different
_types_, such as one email address and one phone number. In this case they will be prompted to choose which address to use during
the recovery flow and only _one_ message will be sent to the address they picked.
