...
define a secondary email field, such as `email_secondary` in the traits section of the schema. This field will serve as the
fallback email address for account recovery.

This is an example of a schema with a secondary email address:
...
messages to both addresses when the recovery flow is initiated. Please note that the user should provide the `email_secondary`
field during registration or update it later in their account settings so it can be used in the recovery process.
