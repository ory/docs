...
- Recovery flows initialized by apps on mobile phones or smart devices don't work with links.
- Depending on the device settings, clicking a link from an email can open it in a different browser than the one used to
  initialize the recovery flow. This can confuse your users.
