...
title: Learn about the flow that allows users to recover access to their accounts and perform password reset
...
The "one-time password" strategy (`code`) is the default, preferred, and recommended by Ory.
...
Consider using the `code` strategy as it mitigates many of the drawbacks of magic links that you must consider:
...
selfservice:
methods:
 code: # Enables "one-time code" strategy. Use 'link' to enable "magic link" strategy.
   enabled: true
   config:
...
flows:
 recovery:
   ...
   use: code # Defines which strategy is used, one of 'code' or 'link'.
```

Both strategies require methods require sending emails to users. This means that you must configure an SMTP server for your instance.
