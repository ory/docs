...
- `passkey` for signing in with a Passkey. Visit the <SameDeploymentLink to="kratos/passwordless/passkeys">Passkey
  guide</SameDeploymentLink>.
- `code` for signing in with a code via Email or SMS. Visit the <SameDeploymentLink to="kratos/passwordless/one-time-code">Code
  via Email / SMS guide</SameDeploymentLink>.
- `webauthn` (legacy) for signing in with Webauthn. This method is supported for backwards compatibility, use
  <SameDeploymentLink to="kratos/passwordless/passkeys">Passkey</SameDeploymentLink> instead.
...
- [Ory OAuth2 & OpenID Connect (Ory Hydra)](https://www.ory.com/hydra);
