...
- `oidc` for un-/linking from social sign in providers such as Google or
  Facebook;
- `profile` for updating an identity's traits (for example change the first
  name). The updated traits must be valid against the Identity Schema defined
  for its [Identity Traits](../../concepts/identity-schema.mdx).
...
This end-user experience works only for Browser-based Settings Flows. API-based
flows will simply return a 403 Forbidden status message which require you to
request a new Ory Kratos Login session using the
...
The Settings Flow for API clients doesn't use HTTP Redirects and can be
...
Social Sign In is not possible for API Clients. It will be possible in a future
...
form (for example updates their first name or email address). Possible
validation errors include JSON Schema validation errors (for example
`"format": "email"` not respected):
...
providing a password which doesn't match the password policy:
...
(for example Google) to be linked is already linked with another identity in the
system.
