...
The new password must be different from the current one. Attempting to reuse a old password will trigger an error: `The new password must be different from the old password.` 

This check ensures that a new, unique password is set, preventing the accidental reuse of a potentially compromised credential.
