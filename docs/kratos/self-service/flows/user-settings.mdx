...
- `oidc` for un-/linking from social sign in providers such as Google or
  Facebook;
- `profile` for updating an identity's traits (for example change the first name). The
...
This end-user experience works only for Browser-based Settings Flows.
...
Social Sign In is not possible for API Clients. It will be possible in
...
form (for example updates their first name or email address). Possible validation
errors include JSON Schema validation errors (for example `"format": "email"` not
respected):
