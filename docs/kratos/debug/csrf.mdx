...
Because Ory Kratos isn't just an API, but instead talks to your users' browsers
...
> Therefore, if the user is authenticated to the site, the site will
...
the browser won't send the cookie unless the URL is a HTTPS URL. If you want
Ory Kratos to work with HTTP (for example on localhost) you can add the `--dev` flag:
`kratos serve --dev`.
...
Cookies work best on the same domain. While it's possible to get cookies
running on subdomains it isn't possible to do that across Top Level Domains
(TLDs).

Make sure that your application (for example the Quickstart self service app ) and Ory
...
As already explained, make sure that the TLD stays the same. This is also
...
JavaScript because the `HttpOnly` flag is set. This flag can't be modified!
...
When implementing a server-side application (for example in PHP, Java, Go, NodeJS, ...)
make sure to include the `Cookie` header from the client when fetching the
self-service flows (for example `GET /self-service/login/flows?id=...`):
...
Without forwarding the `Cookie` HTTP Header it won't be possible to fetch the
