...
Each trait translates into a field presented in an user-facing UI. In the minimum viable identity schema, there are no traits
which means that the UI doesn't present any fields. Let's change that by adding three fields:
...
For example, to add a newsletter opt-in checkbox to the schema, use this code:
...
two true/false fields for specifying the newsletter subscription and enterprise customer status.
