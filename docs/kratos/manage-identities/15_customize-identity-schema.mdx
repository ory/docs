...
The part highlighted below defines the identity's `email` for the email+password flow in The Ory Network. It also includes a method
for [`recovery`](https://www.ory.sh/kratos/docs/self-service/flows/account-recovery#recovery-link-method) as well as
