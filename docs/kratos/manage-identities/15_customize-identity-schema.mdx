...
Use the online [JSON Schema Validator](https://www.jsonschemavalidator.net) to test your identity model and check out some
[JSON Schema examples](https://json-schema.org/learn/miscellaneous-examples.html).

:::

The most minimal form of an identity schema is:
...
## Write your first custom identity schema
...
Each trait translates into a field on the user-facing frontend. Currently, the user has no traits and thus no fields. Let's change
that by adding three fields:
...
![Registration screen with username, first name, and last name.](_static/registration.png)

To log in, a user will need to supply an ID and password. The identity schema does not yet state which field is the ID used for
...
for checkboxes fields, `integer` or `number` for integral or floating-point numbers. If you want to know more about these types,
please refer to the [json-schema documentation](https://json-schema.org/understanding-json-schema/reference/type.html).
...
difficult it is to introduce changes in the future. Complex schemas make it difficult to auto-generate user interfaces.
...
You can configure Ory Identitiesto use specific fields as the identity's identifier. In this example, the `password` is set as the
identifier:
