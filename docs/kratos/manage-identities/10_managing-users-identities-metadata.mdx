...
- **Metadata** are the attributes defined by the system admin that can't be updated or modified by the identity owner. The only
  ...
  such as the original sign up date if the identity was created through a migration. Other use cases include, but are not limited to, 

## Metadata

There are two types of identity metadata:

- **Public:** Attributes that can only be modified using the `/admin/identities` APIs. They are visible to anyone having access to the user's sessions, for example by calling
  `toSession()` or `/sessions/whoami`. This allows you to access the metadata in the frontend without calling the admin APIs.
