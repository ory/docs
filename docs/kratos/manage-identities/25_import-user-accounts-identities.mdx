...
3.  Implement the password migration web hook
    ...
    Identities will call the configured web hook URL with the following payload:
...
Importing SAML is only available on [some plans](https://www.ory.sh/pricing).

:::

When importingSAML connections, the `provider` field is the SAML provider ID you set in your SAML configuration. The `subject` ID
...
When importing SAML / OIDC connections that are only available for certain [organizations](../organizations/organizations.mdx)
