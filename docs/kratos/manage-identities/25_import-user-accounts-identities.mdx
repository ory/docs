...
1.  Import an identity with an empty password hash and `use_migration_hook` set to `true`

    [Bulk-import](#bulk-import-identities-from-other-providers) all your users with an empty password hash and
    `use_migration_hook` set:
...
3.  Implement the password migration web hook
    ...
    If a user logs in and the identifier points to an identity that has `use_migration_hook` set to `true`, Ory Identities will
    call the configured web hook URL with the following payload:
