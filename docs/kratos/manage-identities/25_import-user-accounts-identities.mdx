...
If your system does not have a `subject` ID, you can use the email address as the subject. This is not recommended, as the email
address may change over time, and the user may not be able to log in if the email address changes.

To solve this use case, read the
[automatic account linking on import documentation](../socia-signin/95_account-linking-on-import.mdx#automatic-account-linking).
