...
#### Step 1: Import an identity with an empty password hash and `use_migration_hook` set to `true`

[Bulk-import](#bulk-import-identities-from-other-providers) all your users with an empty password hash and `use_migration_hook`
set:
...
#### Step 2: Configure a password migration web hook

Add the web hook URL to the configuration. See the
[web hook configuration](https://www.ory.sh/docs/guides/integrate-with-ory-cloud-through-webhooks#request-authentication) for
...
#### Step 3: Implement the password migration web hook

If a user logs in and the identifier points to an identity that has `use_migration_hook` set to `true`, Ory Identities will call
the configured web hook URL with the following payload:
...
After a successful response, the identity will be updated with the hashed password and the user will be logged in. The password
migration hook will not be called again for this identity.
