...
When importing clear text passwords, the process may be slower than expected because each password must be securely hashed during
the import process. If you are batch importing a large number of clear text passwords, please hash the passwords before importing
them to avoid timeouts.
