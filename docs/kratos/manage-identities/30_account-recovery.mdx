---
id: account-recovery
title: Learn about the account recovery flow initiated by admins
...
You can generate a link for an account without a recovery address via the admin API, but if the recovery link expires the users
can't re-initiate the flow by themselves if a recovery address hasn't been added.
...
And use it to import an identity:
...
The response contains a `recovery_link` value. This is the link the user should use to set up his or her credentials to connect to
a social sign-in provider or set up a password :
...
The user has a limited amount of time to update their credentials once they use the recovery link. The time is the privileged
session
...
Configure the privileged session lifespan at [console.ory.sh/projects/current/session-settings](https://console.ory.sh/projects/current/session-settings).
...
To enable recovery flows, make the following adjustments to the Ory Identities configuration:
...
For more detailed information and general guidelines on these flows, take a look at the
[account recovery and password reset](../../kratos/self-service/flows/account-recovery.mdx) section.
