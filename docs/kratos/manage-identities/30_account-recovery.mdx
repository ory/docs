...
[account recovery and password reset](../self-service/flows/account-recovery-password-reset.mdx) section.
...
func main() {
	client := ory.NewAPIClient(&ory.Configuration{
		...
		DefaultHeader: map[string]string{
			"Authorization": "Bearer ory_pat_xRKLsFEOUFQFVBjd6o3FQDifaLYhabGd", // Bearer Token for the Ory Network playground project
		},
	})
...
The response contains a `recovery_link` containing the flow ID and a `recovery_code`. The link should be opened by the user which
will show a recovery code form. The `recovery_code` should be entered by the user here.
...
After successfully recovering their account, they can connect to a social sign-in provider or set up a password.
...
For more detailed information and general guidelines on account recovery, take a look at the
[account recovery and password reset](../../kratos//self-service/flows/account-recovery-password-reset.mdx) section.
