...
This guide explains how to configure and use the `external_id` field in Ory Kratos to support custom identity keys such as
`customer_id`, `employee_id`, or similar. This is especially useful for enterprise migrations from systems like ForgeRock,
PingFederate, or Okta that support user-defined primary identifiers.
...
If you're migrating from ForgeRock or a similar IAM that uses domain-specific IDs, you can import identities into Kratos with
their `external_id` using the [batch API](https://www.ory.sh/docs/reference/api#tag/identity/operation/batchPatchIdentities).
...
| Tokenization failure | Occurs if `external_id` is not set but required as `sub` |
...
- [Importing Users in Bulk](../../kratos/manage-identities/import-user-accounts-identities)
