...
1. Inside the emulated device, go to 'Settings > Security & Location > Screen Lock' and set a device PIN (this is required for biometrics).
1. Inside the emulated device, go to 'Settings > Security & Location > Fingerprints' and add a fingerprint. A biometric prompt will appear on the screen of the emulated device.
1. In the 'Extended Controls' for the emulated device (not inside the device, but in Android Studio), go to the 'Fingerprints' section and click on 'Touch sensor' to pass the biometrics prompt of the device. This simulates placing your finger on the sensor.
