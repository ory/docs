...
The passkey strategy exposes additional options that control the WebAuthn ceremony. All options are optional. Defaults accept
every standards-compliant passkey, so existing deployments do not need to change anything.
...
| `timeouts.registration`                     | duration | library default | Timeout for the registration ceremony. Use Go duration format, for example `"60s"` or `"5m"`. When unset, the library default applies: 5 minutes with user verification, 2 minutes without.                      |
