...
Read it together with [Client implementation requirements](index.mdx#client-implementation-requirements) — the comments in the
...
[PIN enrollment](index.mdx#pin-enrollment)), then `openSealedSecret` on the returned `continue_with`, capture the PIN,
`PinVault.seal`, and persist the `PinArtifacts`. Let the `PinCeremony` go out of scope so its transport key is destroyed.
...
challenge computation visible. On iOS, biometric first-factor login also requires the `ios_biometric_first_factor` opt-in — see
[Configuration](index.mdx#configuration).
