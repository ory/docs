...
- **Relaxed-attestation keys stop working** — keys enrolled with relaxed attestation expire after 30 days and are refused as soon
  as the setting is turned off. See [relaxed attestation](08_deviceauthn.mdx#relaxed-attestation-for-testing).
...
| Code execution on the unlocked device                | Can unseal, but a wrong PIN yields garbage; the only oracle is the server, which locks the key after 5 attempts.  |
