...
In Ory Network, the default multi-factor authentication enforcement model is Strict.
...
Follow these steps to adjust the settings in Ory Network using the Ory CLI:
...
When working with self-hosted instances of the Ory Identities (Kratos), change the enforcement model by adjusting these keys
