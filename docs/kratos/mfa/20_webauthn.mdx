...
You can enable WebAuthn in the Ory Identities (Kratos) to allow users to perform 2FA with with physical devices such as USB
...
When working with self-hosted instances of Ory Identities (Ory Kratos), add the `webauthn` method to `selfservice/methods`
