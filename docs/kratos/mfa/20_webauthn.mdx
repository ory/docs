...
You can enable WebAuthn in the Ory Identities (Kratos) to allow users to perform 2FA with with physical devices such as USB keys
or OS-level biometric authentication protocols, such as TouchID.
...
When working with self-hosted instances of Ory Identities (Ory Kratos), add the `webauthn` method to `selfservice/methods` in the
configuration file:
...
This configuration is the default for Ory Network.
...
[you can reference the Cypress tests used in Ory Identities (Kratos)](https://github.com/ory/kratos/blob/fc2cecfbeab811aa1a851f953b6bc2a4c119c412/test/e2e/cypress/integration/profiles/mfa/webauthn.spec.ts).
