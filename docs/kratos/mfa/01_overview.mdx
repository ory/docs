...
SMS-based two-factor authentication sends a one-time password to the user's registered mobile phone number via text message. Read
the [Code via SMS](https://www.ory.sh/docs/kratos/mfa/mfa-via-sms) documentation to learn more.
...
When you enable [passwordless authentication with WebAuthn](../passwordless/01_webauthn.mdx), WebAuthn is not considered as a
