...
**Ory Console** → [**Two-Factor Authentication**](https://console.ory.sh/projects/current/mfa) and use the switches in the
...
Some actions in your application might require a session, with a higher AAL.

To require users to complete a second factor in their current session, initiate a new login flow using one of these endpoints with
the `aal` parameter set to `aal2`:
