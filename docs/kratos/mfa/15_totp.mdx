...
You can enable Time-Based One-Time Password (TOTP) authentication in the Ory Identities (Kratos) to allow users to perform
...
When working with self-hosted instances of the Ory Identities (Ory Kratos), add the `totp` method to `selfservice/methods` in
