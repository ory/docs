...
selected flow. You enrich the flows through hooks.
...
- Registration through a web browser
  ...
  threat agent can distinguish between existing and non-existing accounts by checking if Set-Cookie was sent as part of the registration 
  response.
...
The HTTP response is set by the hook. As a result, no other hooks can be executed after the `session` hook. This is because the initial
HTTP response can't be modified by subsequent hooks. 
