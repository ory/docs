...
- Linking and unlinking social profiles (for example "Sign in with Google" or
  "Connect with Google") to existing accounts
...
Some flows implement the ability [to run hooks](self-service/hooks.mdx) which
allow users to be immediately signed in after registration, notify another
system on successful registration (such as Mailchimp), and so on.
...
All examples use Ory Cloud playground project. Please be aware that everyone can
see the data you enter there. You can replace the URL with a locally hosted Ory
Kratos as well.
...
First, the Browser opens the flow's initialization endpoint (for
...
such as the login methods (for example "username/password" and "Sign in with
Google") and their form data.
...
(for example `selfservice.flows.login.ui_url`), appending the flow ID as the
`flow` URL Query Parameter. Also included are HTTP cookies such as Anti-CSRF
cookies.
...
If a system error (for example broken configuration file) occurs, the browser is
...
Ory Kratos and your UI must be hosted on same top level domain! You can't host
...
If it's, these flows act like an API flow and respond with JSON instead of HTTP
...
The AJAX client (for example NextJS app) makes a request to the flow's
initialization endpoint (for example`/self-service/login/browser`,
...
To submit the form, it's recommended to intercept the submission and send the
form as JSON using `fetch` (see
["Submit HTML Forms to JSON APIs"](https://dev.to/amjadmh73/submit-html-forms-to-json-apis-easily-137l))
or the [SDK](sdk.md).
...
The client (for example mobile application) makes a request to the flow's
initialization endpoint (for example`/self-service/login/api`,
...
The initialization endpoint creates a flow object and stores it in the database.
The flow object has an ID and contains additional information about the flow
such as the login methods and their form data.
