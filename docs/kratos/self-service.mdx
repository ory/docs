...
- Linking and unlinking social profiles (for example "Sign in with Google" or "Connect
...
Some flows implement the ability
...
All examples use Ory Cloud playground project. Please be aware that everyone
...
(for example`/self-service/login/browser`, `/self-service/registration/browser`, ...):
...
such as the login methods (for example "username/password" and "Sign in with Google")
and their form data.
...
(for example `selfservice.flows.login.ui_url`), appending the flow ID as the `flow` URL
...
The AJAX client (for example NextJS app) makes a request to the flow's initialization
endpoint (for example`/self-service/login/browser`,
