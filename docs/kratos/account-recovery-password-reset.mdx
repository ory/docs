...
Account Recovery allows registered users to regain access to their account if they lost their password or multi-factor
authentication (MFA) provider.

## Methods

If a user lost the credentials to their account but still wants to access it, they need to prove ownership of the account. This is
typically done via a recovery address, such as an email address or phone number. By having access to the inboxes of these
addresses, the user proves to the system, that they are the same person, that initially registered the account.

The Ory Identity Service (Ory Kratos) supports two recovery methods:
...
To provide the account recovery mechanism, the Ory Identity Service (Ory Kratos) needs to know which address to send the recovery
message to. In most cases, this is the address that the user used when registering their account. But other fields inside the
`traits` section are supported as well.

:::note

If the email address used for recovery purposes is the same as the email used for verification purposes and the account hasn't
been activated yet (see [Account Activation](./self-service/flows/verify-email-account-activation.mdx)), the address will also be
verified by completing the account recovery successfully.
...
After the user completed the recovery successfully, for example by clicking on the magic link or entering the correct one-time
password, they receive a privileged session and are redirected to the settings page. Here they can update their credentials.

Read more about the [privileged sessions](./session-management/01_overview.mdx#privileged-sessions).

## Invalidate other sessions

In some scenarios it can be useful to deactive all active sessions, once an account has been recovered. This forces anyone that
currently has access to the account to re-authenticate.
...
The Ory Identity Service (Ory Kratos) uses one-time passwords by default. Magic links are available for legacy reasons and will be
...
In stark contrast to other identity systems, the Ory Identity Service (Ory Kratos) doesn't render this HTML directly. Instead, you
need to implement the HTML code in your application (for example in Node.js + Express.js, Java, PHP, React.js, ...), which gives
you extreme flexibility and customizability in your user interface flows and designs. This part of your application then directly
interfaces with the Ory Identity Service by calling different API endpoints.

These in turn respond with a defined JSON document describing the form elements to render and actions the form should take upon
completion, cancellation, etc. The following shows examples for a few different languages and frameworks.
