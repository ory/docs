...
- Other credentials - support other credential types (X509 Certificates,
  Biometrics, ...) that will be added at a later stage.
...
2. an out-of-band one time password (such as sending a login code to your email
   / phone).
3. an OpenID Connect ID token.

It's recommended to reauthenticate the session every 30 days. You can configure
...
It's recommended to reauthenticate the session every 12 hours or after every 30
...
This feature will be implemented at a later stage. Please visit the
[roadmap](../../ecosystem/roadmap.md) for more information on upcoming features.
