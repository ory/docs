...
For other return urls specified inside the configuration it is not necessary to
add the domain to the whitelist.

Let's look at an example, here we are adding `myapp.com` to the whitelist, thus
