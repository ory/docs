...
contains all the claims such as username, email, and others for the OpenID Connect or
OAuth2 Provider. Keep in mind that the claims will vary per provider and per
flow - depending on what permissions the user grants you (for example "App XYZ can't
...
(for example `LOG_LEVEL=debug kratos serve --dev`). Logs with detailed payloads will be
