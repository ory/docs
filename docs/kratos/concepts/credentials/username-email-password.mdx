...
Ory Kratos doesn't hash passwords by SCrypt but can compare stored SCrypt hash and migrate to configured hasher
(`hashers.algorithm`).
...
- `parrelization`(`number`): parallelization parameter (must satisfy r * p < 2³⁰)
- `salt`(`string`): A sequence of bits, known as a cryptographic salt encoded in base64.
