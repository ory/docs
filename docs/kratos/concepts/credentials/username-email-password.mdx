...
Ory Kratos doesn't hash passwords by SCrypt but can compare stored SCrypt hashes and migrate to configured hasher
(`hashers.algorithm`).

#### Format

```
$scrypt$N=<cost>,r=<block>,p=<parrelization>$<salt>$<hash>
```
...
- `parellization`(`number`): parallelization parameter (must satisfy r \* p < 2³⁰)
- `salt`(`string`): A sequence of bits, known as a cryptographic salt encoded in base64.
