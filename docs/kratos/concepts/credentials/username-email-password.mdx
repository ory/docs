...
(`hashers.algorithm`). You can configure MD5 hashes to use a plain format (for hashes without salt) or a salted format (for hashes with salt).
...
If the salting should be performed before computing the MD5 hash, use this format:
