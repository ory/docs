...
On Linux 5.13 and later, Ory Identities applies a [Landlock filesystem
sandbox](../../security-compliance/landlock-sandbox.mdx) to limit what the process can read or execute at runtime.
Ory Network and OEL deployments sandbox the full `kratos serve` process; the OSS binary sandboxes the hidden Jsonnet
worker so mapper code cannot reach the filesystem under any circumstances.
