...
Ory Identities supports BCrypt for password hashing per default. BCrypt is recommended when you want hashing to take at most one
second:
...
Ory Identities supports Argon2 for password hashing. Argon2 is recommended when you want hashing to take at least 2 seconds. You
can tweak the Argon2 configuration in your Ory Identities configuration file:
...
To prevent weak passwords Ory Identities implements different measures. Users often choose passwords similar to their traits. To
prevent this Ory Identities ensures there is a sufficient
[Levenshtein-Distance](https://en.wikipedia.org/wiki/Levenshtein_distance) (aka "Edit-Distance") between the identifier and the
password. It also makes sure that the identifier and password have a small enough longest common substring.
...
one. To protect the value of the password the [range API](https://haveibeenpwned.com/API/v3#SearchingPwnedPasswordsByRange) is
being used.
