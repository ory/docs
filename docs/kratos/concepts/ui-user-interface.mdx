...
Ory Kratos is an HTTP API server and doesn't include a traditional HTML user
...
(for example "login" screen, "update profile settings", ...) in any programming
...
forms that need to be shown during login, registration, and other flows:
...
Rendering it's straight forward if you are on the server-side:
...
Nodes of type `a` are not used in Ory Kratos, but we've them around in case
they're needed at a later stage!
...
Nodes of type `text` are commonly used to display secrets (for example lookup
secrets).
...
```json
{
  ...
  "meta": {
    "label": {
      "id": 1050006,
      "text": "This is your authenticator app secret. Use it if you can't scan the QR code.",
      "type": "info"
    }
  }
}
...
The most common node type is the `input` type. It represents an HTML `<input>`
field:
...
In single page apps, such as ReactJS, this trickery isn't needed as you have
...
Buttons are primarily used to trigger something, which is why it's important to
...
In a single page application, we need a bit of trickery as we're executing
JavaScript from a JSON source in JavaScript - which essentialy means we need to
execute `eval`. This is only needed in this special case. If you do not use
WebAuthn, you won't need `eval` at all!
...
that the name of the form (`method`) is only submitted once with the correct
value (`password`). Because in some cases Ory Kratos might generate multiple
submit buttons with the same `name` attribute.
...
You don't need to have one view for every node type. Instead, you can "fall
...
Ory Kratos helps users understand what's happening by providing messages that
...
{
  id: 1234,
  // This ID won't change and can be used to translate the message or use your own message content.
  text: 'Some default text',
  // A default text in english that you can display if you don't want to customize the message.
  context: {}
...
- `x` is the message type which is either `1` for an info message (for example
  `1020000`), `4` (for example `4020000`) for an input validation error message,
  and `5` (for example `5020000`) for a generic error message.
- `yy` is the module or flow this error references and can be:
- `01` for login messages (for example `1010000`)
- `02` for logout messages (for example `1020000`)
- `03` for multi-factor authentication messages (for example `1030000`)
- `04` for registration messages (for example `1040000`)
- `05` for settings messages (for example `1050000`)
- `06` for account recovery messages (for example `1060000`)
- `07` for email/phone verification messages (for example `1070000`)
...
###### Please confirm this action by verifying that it's you. (1010003)
...
###### This is your authenticator app secret. Use it if you can't scan the QR code. (1050006)
...
###### You successfully recovered your account. Please change your password or set up an alternative login method (for example social sign in) within the next 0.02 minutes. (1060001)

```json
{
  "id": 1060001,
  "text": "You successfully recovered your account. Please change your password or set up an alternative login method (for example social sign in) within the next 0.02 minutes.",
  "type": "info",
  "context": {
...
###### "{value}" isn't valid "{format}" (4000004)

```json
{
  "id": 4000004,
  "text": "\"{value}\" isn't valid \"{format}\"",
  "type": "error",
  "context": {
    "actual_value": "{value}",
...
###### The password can't be used because {reason}. (4000005)

```json
{
  "id": 4000005,
  "text": "The password can't be used because {reason}.",
  "type": "error",
  "context": {
    "reason": "{reason}"
  }
...
###### Couldn't find any login identifiers. Did you forget to set them? (4000009)

```json
{
  "id": 4000009,
  "text": "Couldn't find any login identifiers. Did you forget to set them?",
  "type": "error"
}
```
...
###### The backup recovery code isn't valid. (4000012)

```json
{
  "id": 4000012,
  "text": "The backup recovery code isn't valid.",
  "type": "error",
  "context": {}
}
```
...
###### Couldn't find a strategy to log you in with. Did you fill out the form ? (4010002)
...
###### Couldn't find a strategy to sign you up with. Did you fill out the form ? (4010003)

```json
{
  "id": 4010003,
  "text": "Couldn't find a strategy to sign you up with. Did you fill out the form ?",
  "type": "error"
}
```

###### Couldn't find a strategy to update your settings. Did you fill out the form ? (4010004)

```json
{
  "id": 4010004,
  "text": "Couldn't find a strategy to update your settings. Did you fill out the form ?",
  "type": "error"
}
```

###### Couldn't find a strategy to recover your account with. Did you fill out the form ? (4010005)

```json
{
  "id": 4010005,
  "text": "Couldn't find a strategy to recover your account with. Did you fill out the form ?",
  "type": "error"
}
```

###### Couldn't find a strategy to verify your account with. Did you fill out the form ? (4010006)

```json
{
  "id": 4010006,
  "text": "Couldn't find a strategy to verify your account with. Did you fill out the form ?",
  "type": "error"
}
```
...
###### The request was already completed successfully and can't be retried. (4060001)

```json
{
  "id": 4060001,
  "text": "The request was already completed successfully and can't be retried.",
  "type": "error",
  "context": {}
}
```
...
###### The request was already completed successfully and can't be retried. (4070002)
