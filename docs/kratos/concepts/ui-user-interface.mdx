...
Nodes of type `a` are not used in Ory Kratos, but we've them around
...
Nodes of type `text` are commonly used to display secrets (for example lookup secrets).
...
The most common node type is the `input` type. It represents an HTML
`<input>` field:
...
execute `eval`. This is only needed in this special case. If you do
not use WebAuthn, you won't need `eval` at all!
...
that the name of the form (`method`) is only submitted once with the
correct value (`password`). Because in some cases Ory Kratos might generate
...
3. At the field level, indicating that the message affects a form field (for example
   validation errors).
...
- `x` is the message type which is either `1` for an info message (for example
  `1020000`), `4` (for example `4020000`) for an input validation error message, and
  `5` (for example `5020000`) for a generic error message.
