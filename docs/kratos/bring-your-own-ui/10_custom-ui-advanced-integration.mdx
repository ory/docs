...
The `return_to` URL is the redirect URL after the flow is completed. For example, in the case of a login flow the redirect happens
...
In some cases, it is necessary to pass additional parameters to the upstream provider. For example, when using the Google
provider, it is possible to pass the `login_hint` and `hd` parameters.
...
proper session. The `hd` parameter limits the login/registration process to a Google Organization, e.g. `mycollege.edu`.

To pass these parameters on `login`, `registration` and `settings` flows, the application can use the submit form body to pass the
...
Only the `login_hint` and `hd` parameters are currently supported. More parameters can be supported in the future.
