...
In some cases, it is possible to pass additional parameters to the upstream provider. For example, when using the Googleprovider,
it is possible to pass the `login_hint` and `hd` parameters.
...
proper session. The `hd` parameter limits the login/registration process to a Google Organization, for example `mycollege.edu`. To
pass these parameters on `login`, `registration` and `settings` flows, the application can use the submit form body to pass the
...
Only the `login_hint` and `hd` parameters are currently supported.
