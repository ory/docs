...
title: Advanced information you need to integrate your UI with Ory
...
user interface. The topics range from advanced redirects to passwordless and two-factor authentication. Read the
...
The `return_to` URL will is as the redirect URL after the flow is completed. For example, in the case of a login flow the redirect
...
Ory to force user to authenticate again through the `?refresh=true` query parameter. To do that, call the
...
redirected. For the application to then have a background POST on other methods such as `password`, the application must the flow
...
Two-factor authentication (2FA) is a second step used to confirm user's identity. When creating a login flow, you can specify the
`?aal=<level>` query parameter to specify the required level of authentication: `aal1` or `aal2`.

To initialize second authentication factor, the user must already have a valid session cookie. The
...
Read [this document](./25_ui-nodes-errors.mdx) to learn more about UI error messages.
...
4. The form is submitted which starts a new flow with the `webauthn` group only.
...
After the user is redirected with a new flow, the response contains the `webauthn` group only. In this response, the button
`onclick` attribute calls the script added in the header of the page. Triggers the WebAuthn prompt for the security key.
...
In the case of a SPAs, you can get a code `422` response which indicates that the browser needs to be redirected with a newly
created flow. Since this is an SPA, the new flow ID can be extracted from the payload and the response can be retrieved in the
background instead of a redirect.
