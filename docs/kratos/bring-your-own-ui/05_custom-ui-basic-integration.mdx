...
the same time to allow users to choose the `method` they wish to use. Some flows can also be disabled, for example the
`registration` flow can be disabled if the intent is to have an invite only registration process. These features allow Ory to be
very flexible and support a variety of use cases.
...
**Native applications** are all other applications that are not rendered in the browser. For example an Android or Desktop
application. Since the application is not rendered in the browser, the CSRF cookie and token are not necessary.
...
Below there are code examples to create a `registration` flow and render the user interface (where applicable).
When creating a `registration` flow for **browser applications** you must use the [`/self-service/registration/browser`](../reference/api#tag/frontend/operation/createBrowserRegistrationFlow) endpoint.
When creating a `registration` flow for **native applications** you must use the [`/self-service/registration/api`](../reference/api#tag/frontend/operation/createNativeRegistrationFlow) endpoint.
...
When a flow already exists, we can retrieve it through a GET request to the
[`/self-service/registration/flows?id=<flow-id>`](../reference/api#tag/frontend/operation/getRegistrationFlow)
endpoint using the flow ID. Usually the flow ID is stored in the URL query parameter `?flow=` in your application.
Below is an example of how to retrieve the flow data using the flow ID.
...
The last step is to submit the `registration` flow with the provided user credentials and traits.
This is done through a POST request to the Ory [`/self-service/registration`](../reference/api#tag/frontend/operation/updateRegistrationFlow) endpoint.
...
provide in the POST request payload. To learn more about customizing your Identity Schema please refer to
[Identity model](../manage-identities/overview) documentation.

For browser applications you need to ensure that all cookies are included in the request and that the CSRF token is sent in the body of the request.
The CSRF token value is a hidden input field called `csrf_token`.

Based on the following Identity Schema snippet the examples below submit the required traits `traits.email` and `traits.tos`.
...
The `recovery` flow is used to recover user credentials and is used in conjunction with the `settings` flow.
Once a `recovery` flow has been submitted successfully, a session is issued to the application.
This allows the user to reset their password and update their profile information on the settings page.
Browser applications are automatically redirected to the settings page, while native applications will need to
navigate the user to the settings page.

The `recovery` flow has two methods, `link` and `code`. The `link` method requires the user to open the recovery link
from their email client using a browser on the same device. This makes it difficult as the user might be switching
between devices to recover their credentials.
...
This method is also more preferred due to spam filters and email clients invalidating the recovery link.

:::note

Only one recovery method can be enabled at a time. The `code` method is enabled by default on all new Ory Network projects.

:::

### Create Recovery flow

Below there are code examples to create a recovery flow and render the user interface (where applicable).
When creating a recovery flow for **browser applications** you must use the
[`/self-service/recovery/browser`](../reference/api#tag/frontend/operation/createBrowserRecoveryFlow) endpoint.
When creating a recovery flow for **native applications** you must use the [`/self-service/recovery/api`](../reference/api#tag/frontend/operation/createNativeRecoveryFlow) endpoint.
...
When a flow already exists, we can retrieve it through a GET request to the
[`/self-service/recovery/flows?id=<flow-id>`](../../reference/api#tag/frontend/operation/getRecoveryFlow) endpoint using the flow ID.
Usually the flow ID is stored in the URL query parameter `?flow=` in your application. Below is an example of how to retrieve the
flow data using the flow ID.
...
The next step is to submit the recovery flow with the user's email. This is done through a POST request to the Ory
[`/self-service/recovery`](../../reference/api#tag/frontend/operation/updateRecoveryFlow) endpoint.

For browser applications you need to ensure that all cookies are included in the request and that the CSRF token is sent in the
body of the request. The CSRF token value is a hidden input field called `csrf_token`.

The `recovery` flow could have a second submit step if the recovery method is a `code` strategy.
In this case after the user has submitted their email the recovery flow will include a field to submit the received code.
...
The `verification` flow is used to verify the user's account through email. In some cases applications might need
to restrict access to the application or certain features until the user has verified that they own the email address.

The `verification` flow has two methods, `link` and `code`. The `link` method requires the user to open the verification link
from their email client using a browser on the same device. This makes it difficult as the user might be switching
between devices.

The `code` method is more user friendly since the code can be entered on the same device where the user requested the code from.
This method is also more preferred due to spam filters and email clients invalidating the verification link.

Once the `verification` flow has been submitted successfully, the session will contain the
`identity.verifiable_addresses[0].verified: true` value.

:::note

Only one verification method can be enabled at a time. The `code` method is enabled by default on all new Ory Network projects.

:::

### Create Verification flow

Below there are code examples to create a verification flow and render the user interface (where applicable).
When creating a verification flow for **browser applications** you must use the
[`/self-service/verification/browser`](../reference/api#tag/frontend/operation/createBrowserVerificationFlow) endpoint.
When creating a verification flow for **native applications** you must use the [`/self-service/verification/api`](../reference/api#tag/frontend/operation/createBrowserVerificationFlow) endpoint.
...
When a flow already exists, we can retrieve it through a GET request to the
[`/self-service/verification/flows?id=<flow-id>`](../../reference/api#tag/frontend/operation/getVerificationFlow) endpoint using the flow ID.
Usually the flow ID is stored in the URL query parameter `?flow=` in your application. Below is an example of how to retrieve the
flow data using the flow ID.
...
The next step is to submit the verification flow with the user's email. This is done through a POST request to the Ory
[`/self-service/verification`](../../reference/api#tag/frontend/operation/updateVerificationFlow) endpoint.

For browser applications you need to ensure that all cookies are included in the request and that the CSRF token is sent in the
body of the request. The CSRF token value is a hidden input field called `csrf_token`.

The `verification` flow could have a second submit step if the verification method is a `code` strategy.
In this case after the user has submitted their email the verification flow will include a field to submit the received code.
