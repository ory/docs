...
Examples in this section are based of the following identity schema snippet and submit the required traits
...
The flow has two methods: `link` and `code`. The `link` method requires the user to open the verification link from their email client
using a browser on the same device. This makes it difficult as the user might be switching between devices to verify their account.
