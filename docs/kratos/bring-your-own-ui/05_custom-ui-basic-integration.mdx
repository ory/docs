...
- Native applications, such as Android mobile apps or desktop applications, are not rendered in the browser. Since the application
  is not rendered in the browser, the CSRF cookie and token are not necessary.
...
On successful login, Ory issues a session token that can be used to authenticate subsequent requests to Ory.
...
For more complex use cases you can pass additional data to the flow by utilising the optional `transient_payload` field. This data
gets forwarded to webhooks without beeing persisted by Ory like identity traits do. To learn how to configure and use a webhook,
please have a look at the [Webhooks](../../guides/integrate-with-ory-cloud-through-webhooks.mdx) documentation.
...
Examples in this section are based of the following identity schema snippet and submit the required traits `traits.email` and
`traits.tos`. Also an example `transient_payload` was added to demonstrate it's usage.
