...
title: Custom user interface with Ory Elements
...
Ory Elements is a component library designed to make building login, registration, and account pages for Ory straightforward. Ory
...
changes to after you edit the Identity Schema without the need for any other action. The same is true for other options available
in Ory Network, such as social sign-in connections or passwordless login.
...
3.  In a new terminal window, run Ory Tunnel (part of the Ory CLI) to connect the application to your Ory Network project:
...
4.  In your browser, go to [http://localhost:3000](http://localhost:3000) to access the application. From there, you can try
    registering and signing-in using the Ory Elements-powered pages.
...
With the application running have a closer look at the code and the way the application integrates with Ory Elements and Ory SDK.

### Ory SDK instance

The `sdk.ts` file creates a new instance of the Ory SDK. This instance is used to interact with Ory APIs.
...
they're redirected to the login page. The logout URL is also initialized, which allows for a functional **Log out** button.
...
disabled, the option to add hardware keys isn't visible in the UI.
