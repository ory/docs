...
functions to test the login, registration, recovery, verfication and settings flows. In these examples, the tests are located in
the `e2e` folder and are based on the [Playwright](https://playwright.dev/) library.

To run the tests, you can run them through the `npm run test` command inside the example.
