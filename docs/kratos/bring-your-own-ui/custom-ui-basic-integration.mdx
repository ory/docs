...
Once a recovery flow has been submitted successfully, a session issued to the application.
...
The `code` method is more user friendly since the code can be entered on the same device as the one they requested
the recovery code from. This method is also more preferred due to spam filters and email clients invalidating the
recovery link.
...
When creating a registration flow for **native applications** you must use the [`/self-service/recovery/api`](../reference/api#tag/frontend/operation/createNativeRecoveryFlow) endpoint.
