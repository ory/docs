...
This quickstart guide operates on the assumption that we're writing a NodeJS
...
you name it! We picked NodeJS + TypeScript because we believe it's the easiest
...
implement that yourself or choose Ory Cloud offering (to be announced). In this
quickstart we will use Ory Kratos' CLI to interact with Ory Kratos' APIs.
...
[Step-by-step guides](../guides/social-signin/overview) to add sign up and login
with popular OIDC providers to the Ory Kratos Quickstart, for example:
