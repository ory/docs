...
This guide explains how to setup and run Ory Kratos in an exemplary production
environment. It uses Postgres as database, Nginx as reverse proxy,
[Digital Ocean](https://www.digitalocean.com/) as cloud provider, and the Ory
Kratos NodeJS UI Reference as user interface. You can use another
[relational database](https://www.ory.sh/docs/ecosystem/deployment#data-storage-and-persistence),
a different reverse proxy, deploy on any other cloud host, and
[spin up a custom interface in your favorite language](https://www.ory.sh/docs/kratos/sdk/) -
...
- Installation and configuration of
  [Ory Kratos NodeJS UI Reference](https://github.com/ory/kratos-selfservice-ui-node)
...
of a virtual machine (VM) may vary depending on the scale of your application.

:::

Wait for the virtual machine (VM) to start up and copy the IP address.

This guide uses `accounts.example.com` as a hostname to run Ory Kratos. Replace
it with `<something>.<your-domain>` for your purposes. You need to configure
DNS, create an `A type` record, and point it to the VM's IP.

## Install required dependencies

Connect to your
[droplet via SSH](https://docs.digitalocean.com/products/droplets/how-to/connect-with-ssh/)
or use the
...
The default version of [Node.js](https://nodejs.org/en/) is v10.19.0 on Ubuntu
20.04, we need to install a newer version. You can use the following script to
install Node.js 16.x on a Ubuntu system.
...
encryption. Open the file at `/etc/postgresql/12/main/pg_hba.conf`, and add the
following content:
...
Type or paste the password for the Ory Kratos user we created before. If
everything works correctly you should see a prompt similar to this:
...
Congratulations, you have installed and configured the PostgreSQL database to
store your identities. Next we will setup Ory Kratos.
...
wget https://github.com/ory/kratos/releases/download/${useLatestRelease(
  'kratos'
)}/kratos_${useLatestReleaseFilename('kratos')}-linux_64bit.tar.gz
# Extract contents
...
We need configure Ory Kratos to use the Postgres database. Open `kratos.yml` and
change the dsn configuration. By default `dsn: memory` is configured, so Ory
...
[`serve` command](../cli/kratos-serve) from earlier.
...
Check running processes with `ps ax | grep kratos`. If everything worked
...
Check if it's accessible (if your DNS record is not set yet (it takes up to 24h
to propagate), you can also send the request here directly to your droplet IP)
...
`serve.public.host` and `serve.admin.host` to `127.0.0.1` to ensure Ory Kratos
is listening on the loopback interface.

Change the following sections in your `kratos.yml` (in case you forgot: you can
...
We'll use Nginx as reverse proxy and load balancer for our service. You can use
another reverse proxy and load balancer instead. Install Nginx by running:
...
`accounts.example.com` at `/etc/nginx/sites-available/` with the following
content
...
Configure SSL via Certbot. Run the following command and answer the questions to
set it up. When prompted choose to redirect HTTP traffic to HTTPS.
...
There are some missing parts in your configuration at this point, and we need to
configure locations and
[upstream APIs](http://nginx.org/en/docs/http/ngx_http_upstream_module.html).
You can balance network traffic between different instances of Ory Kratos
running on the various virtual machines. We need two upstream APIs:
...
`/etc/nginx/sites-enabled/accounts.example.com` file
...
Add your locations and the `/etc/nginx/sites-enabled/accounts.example.com` has
...
cd /opt/uinode
wget https://github.com/ory/kratos-selfservice-ui-node/archive/refs/tags/${useLatestRelease(
  'kratos'
)}.zip
...
Create a configuration file for systemd. Create a file named `uinode.service` at
`/etc/systemd/system/` with the following content (change
`https://accounts.example.com` to your domain!):
...
In `kratos.yml` change URLs to use the user interface you installed in the step
before:
...
Open `https://accounts.example.com/auth/login` and you should see the Login
Screen.

## Final steps

We covered a basic guide for deploying Ory Kratos to production with Nginx. However, the configuration of Kratos itself is not production-ready. We need to do the following things

1. Setup secure keys
1. Run Ory Kratos in production mode

You can generate secure keys using the following `openssl` command
...
```
with unique and generated strings like in example below
...
And don't forged to change `log.leak_sensitive_values` to false

```
log:
  leak_sensitive_values: false
```
Save changes and restart Ory Kratos by running
...
- Fork the
  [Ory Kratos NodeJS UI Reference](https://github.com/ory/kratos-selfservice-ui-node)
  or
  [build a custom UI in the language of your choice](../sdk.md).
- [Add Two Factor Authentication (2FA) to your App](./two-factor-authentication-2fa-mfa.mdx)
