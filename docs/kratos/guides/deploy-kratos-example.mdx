...
We covered a basic deployment of Ory Kratos to production with Nginx. However,
the configuration of Kratos itself is not production-ready. We need to:

1. Set up secure keys.
1. Run Ory Kratos in production mode.

Generate secure keys using the following
[`openssl`](https://www.openssl.org/docs/manmaster/man1/openssl.html) command:
...
Change `log.leak_sensitive_values` to false, to not leak any sensitive values
...
- Fork the
  [Ory Kratos NodeJS UI Reference](https://github.com/ory/kratos-selfservice-ui-node)
  or [build a custom UI in the language of your choice](../sdk/index.md).
