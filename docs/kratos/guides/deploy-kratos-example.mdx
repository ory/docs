...
[relational database](../../oss/deployment.mdx#data-storage-and-persistence), a different reverse proxy, deploy on any other cloud
host, and [spin up a custom interface in your favorite language](../sdk/overview) - this is just an example!
...
- Review the [Landlock filesystem sandbox](../../security-compliance/landlock-sandbox.mdx) — Ory Network and OEL
  binaries restrict `kratos serve` to its config, TLS, schema, courier template, and SQLite database paths at runtime.
