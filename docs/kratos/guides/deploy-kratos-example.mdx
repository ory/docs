...
We need to change owner of `/opt/kratos` directory to `kratos` user:

```bash
chown -R kratos /opt/kratos/
```

Create a `/etc/systemd/system/kratos.service` file with the following content:
...
Running Ory Kratos using systemd:
...
We have Ory Kratos up and running, but we need to make the Kratos Admin API  inaccessible via the
public internet and configure a reverse proxy. We need to set `serve.public.host`
and `serve.admin.host` to `127.0.0.1` to ensure Ory Kratos is listening
