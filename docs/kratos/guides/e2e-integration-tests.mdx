...
[NodeJS](https://github.com/ory/kratos-selfservice-ui-node) and
[React Native](https://github.com/ory/kratos-selfservice-ui-react-native)
reference implementation using [Cypress](https://www.cypress.io). You can find
the set up and source code for these tests in
[./test/e2e/run.sh](https://github.com/ory/kratos/blob/master/test/e2e/run.sh).
...
as well as our application in the background as well. Then, we start the Cypress
test runner which executes the different e2e tests. This works really well to
test compliance and integration of Ory Kratos.

If you run Ory Kratos and your app on separate domains or ports you might want
to add
...
If you want to test React Native (rendered as a web application) in Cypress, you
...
`DEV_DISABLE_API_FLOW_ENFORCEMENT=1`:
