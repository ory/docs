...
At the moment it isn't possible to set up Ory Kratos in a way where you get session cookies running on two separate top level
domains (for example `my-domain.com` and `another-domain.com`). This is supported only on Ory Network through the
[Multibrand](https://www.ory.sh/docs/kratos/bring-your-own-ui/configure-ory-to-use-your-ui#multiple-custom-domains) feature.
