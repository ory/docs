...
- Tracing backend(s): Jaeger - _Note:
...
- Following existing traces: If you have deployed Kratos behind a proxy that has
  ...
  request headers for tracing context. At present, support for Jaeger
  and Zipkin (B3) spans exists; support for additional formats like Trace Context
  and Baggage is planned.
