...
- Tracing backend(s): Jaeger
  ...
  **Note**: Adding support for other
  [OpenTelemetry exporters](https://opentelemetry.io/docs/instrumentation/go/exporting_data/)
  is planned. To aid in priority, please
  [create an issue](https://github.com/ory/kratos/issues) with your feature
  request. However, official support in OpenTelemetry only exists for the
  exporters listed
  ...
  OpenTelemetry Collector agent, and configuring the collector to push spans to
  whatever
...
- Following existing traces: If you have deployed Kratos behind a proxy that has
  ...
  request headers for tracing context. At present, support for Jaeger and Zipkin
  (B3) spans exists; support for additional formats like Trace Context and
  Baggage is planned.
...
The
[provided docker-compose file](https://github.com/ory/kratos/blob/master/quickstart-tracing.yml)
