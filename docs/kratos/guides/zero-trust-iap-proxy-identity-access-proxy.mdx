...
Systems that have more than one component often use reverse proxies such as
...
only works if one isn't yet authenticated.
...
- An example application written in NodeJS that implements the login,
  registration, logout, dashboard, and other UIs. Because we're accessing this
  via a proxy, its port (4435) isn't publicly exposed.
...
is useful for endpoints that don't need login, such as the registration screen:
