...
- using an Operating System "platform module" (for example TouchID, FaceID,
  Windows Hello Face, Android Biometric Authentication, ...)

Configuring WebAuthn is imperative, because the authentication will fail if
something isn't configured.
...
- WebAuthN is limited to one domain and doesn't work in a local environment when
  using CNAME / Ory Proxy. WebAuthN uses an `https://origin` URL as part of the
  client<->server challenge/response mechanism. This mechanism allows for only
  one URL as the origin. Read more in the
