...
- `aal2`: Implies that the identity has completed the first (`password`, `oidc`)
  ...
  (for example: `password` + `totp`, `oidc` + `webauthn`, ...). First authentication
  factors can't be combined to gain `aal2` (for example `password` + `oidc` = `aal1`,
  not `aal2`)!
...
- `highest_available` (default): If set, requires identities who have set up a
  second factor (for example `totp`, `webauthn`) to have an Ory Session where both
  factors (for example `password` + `totp`) have been used to authenticated.
...
their second factor, the method (for example `totp`) will be added to the Or Session
...
end-user can unlink the TOTP device if for example it's lost:
...
- using a USB, NFC, or Bluetooth low energy device (for example
  [YubiKey](https://www.yubico.com)) to authenticate
- using an Operating System "platform module" (for example TouchID, FaceID, Windows
  Hello Face, Android Biometric Authentication, ...)

Configuring WebAuthn is imperative, because the authentication will
fail if something isn't configured.
...
- WebAuthN is a Browser standard. It doesn't work on native mobile apps.

- WebAuthN is limited to one domain and doesn't work in a local environment
