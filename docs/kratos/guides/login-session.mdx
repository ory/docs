...
returns a list of all sessions that are active and haven't expired, except the
current session. This can be used to show a UI with all other sessions that are active.
...
Sessions that were revoked through self-service aren't deleted. Instead, they
