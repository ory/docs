...
Kratos operates in such an environment and it can't communicate with the HiBP
...
Alternatively, this feature can be disabled altogether. Note however that by
doing so you will significantly decrease the security of the service and not
follow [NIST guidelines](https://pages.nist.gov/800-63-3/sp800-63b.html):
...
Disabling the feature will effectively remove checks for the points 1.-3. The
feature can be disabled by setting the following configuration values:
...
Ory Kratos `haveibeenpwned_host` setting doesn't allow configuring the
