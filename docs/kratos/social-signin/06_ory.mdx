...
To add Ory Network a social sign-in provider, you need to create an OAuth2 Client:
...
Follow these steps to add Ory as social sign-in provider to your project using the Ory Console:
...
When adding Ory as social sign-in provider, you can customize the integration by defining the OAuth scopes Ory requests from
...
local claims = std.extVar('claims');
{
  identity: {
    traits: {
      [if 'sub' in claims then 'email' else null]: claims.sub,
    },
  },
}
```
...
1. Create a client at your generic provider to get a Client ID and Client Secret. TODO Hydra
         ...
         config:
           providers:
             - id: generic # this is `<provider-id>` in the Authorization callback URL. DO NOT CHANGE IT ONCE SET!
               ...
               issuer_url: https://<your-ory-project>.projects.oryapis.com/ # Replace this with your Ory Network Project URL
