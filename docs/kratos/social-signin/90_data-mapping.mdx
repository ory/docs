...
the Identity Service configuration for the given social sign-in provider.
...
Ory Identity Service (Ory Kratos) adds an external variable called `claims` to the data mapper. It contains all claims for the
OpenID Connect or OAuth2 provider such as the username and user email.
...
All claims that are not part of the standard userinfo claims, are mapped into raw_claims. An example of mapping a Raw Claim called
"groups":
