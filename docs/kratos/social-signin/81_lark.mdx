...
1. create an application in [Feishu Open Platform Developer](https://open.feishu.cn/app) Background and obtain the ID and secret of the application.
         ...
         config:
           providers:
             - id: lark # this is `<provider-id>` in the Authorization callback URL. DO NOT CHANGE IT ONCE SET!
               ...
               # Lark use fixed scope for application which should be configured in it's admin page. Thus you can use an empty scope list here.
               scope: []
   ```
