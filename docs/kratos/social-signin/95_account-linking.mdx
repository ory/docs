...
1. When signing in later, the user signs in with a SSO login provider. That SSO account (through the OIDC userinfo endpoint or the
   ID token) returns a **verified email** of `alice@example.com`.
1. Since the verified address already exists, the user will be prompted to enter the password chosen in step 1.
