...
1. Since the identifier already exists, the user cannot be logged in directly. Instead, the user will be prompted to enter the
...
### Hiding irrelevant authentication methods

Due to security considerations, it might be required to show all configured authentication methods to the user. However, this can
be confusing for users who have multiple accounts with different authentication methods.

To hide irrelevant authentication methods, you can use the [login hints](../../identities/sign-in/login-hint.mdx) feature. By
enabling login hints, users are informed about the authentication methods available for the existing account.
