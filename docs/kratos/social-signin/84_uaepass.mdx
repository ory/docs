...
UAE PASS supports two authentication flows: web (default) and mobile. The provider defaults to the web flow. To use the
mobile app-to-app flow, pass the `acr_values` upstream parameter at request time.
...
To trigger the mobile flow, include `acr_values` in the `upstream_parameters` when initiating the OIDC login or
registration flow:
