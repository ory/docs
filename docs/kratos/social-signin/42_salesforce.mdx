...
4. [Create an Salesforce Connected App](https://help.salesforce.com/s/articleView?id=sf.service_provider_define_oid.htm&type=5), noting the following points:
...
6. Ensure the `Require Secret for Web Server Flow` checkbox is enabled.
7. Create the app.
8. Go to the **OAuth Settings** of your newly created connected app, click on **Manage Consumer Details** and copy the following data to the
...
9. Copy the Salesforce top-level domain from the URL bar (typically
   `https://mySalesforceTenant.my.salesforce.com`) to the **Tenant URL** field of the form in the Ory Console.
10. In the **Data Mapping** field of the form in the Ory Console, add the following Jsonnet code snippet, which maps the desired
...
4. Encode the Jsonnet snippet with [Base64](https://www.base64encode.org/) or host it under a URL accessible to Ory Network.
