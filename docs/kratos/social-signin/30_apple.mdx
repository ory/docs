...
Apple supports [automatic account linking](./link-multiple-provider-account#automatic-account-linking-for-google-and-apple). When
enabled, users who sign in with an Apple account that shares a verified email with an existing identity are automatically linked
