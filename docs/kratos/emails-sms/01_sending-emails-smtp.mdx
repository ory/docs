...
The courier passes the `Recipient`, `TemplateType`, and `TemplateData` variables into the Jsonnet template via the `ctx`
parameter. `Recipient` will always be the email address of the user. `TemplateData` contains the same variables, described in the
[custom email templates page](./05_custom-email-templates.mdx#available-variables).
