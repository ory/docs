...
When using your own template, map top-level fields such as `recipient` and `template_type` directly from `ctx`. Map the other
fields your template needs from `ctx.template_data`.
