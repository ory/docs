...
The `body: file://...` Jsonnet template path is not auto-discovered by the
[Landlock filesystem sandbox](../../security-compliance/landlock-sandbox.mdx). Add it to
`security.landlock.allowed_paths` so the courier can read it at send time.
