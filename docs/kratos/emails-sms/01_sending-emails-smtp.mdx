...
Ory supports sending emails using your own SMTP server.
...
To configure your credentials for the custom SMTP server, go to **Customize** → **Email Configuration** in the
[Ory Console](https://console.ory.sh/projects/current/email-configuration).
...
Customizing the `from_address` and `from_name` is not supported without a custom SMTP server.
...
If you have problems setting up email delivery, you can view outgoing messages on the **Monitoring** → **Email Delivery** page in
the [Ory Console](https://console.ory.sh/projects/current/email-delivery)
