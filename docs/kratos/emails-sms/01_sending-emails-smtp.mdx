...
The `ctx` properties are defined by the `httpDataModel` struct in Ory Kratos.
...
The following is the mailer configuration template shipped with Ory Kratos. It maps the most common fields from `ctx`.
...
When using your own template, map top-level fields such as `recipient` and `template_type` directly from `ctx`. Map the other fields your template needs from `ctx.template_data`.
