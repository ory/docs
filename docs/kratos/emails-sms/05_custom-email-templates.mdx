...
1. `recovery_code` - the system sends these messages when users recover their accounts, for example after forgetting their password.
   - `recovery_code.valid` - the system sends these messages when the user provided a valid recovery address.
   - `recovery_code.invalid` - the system sends these messages when the recovery flow fails, for example when you start the recovery
     flow using an email address that isn't associated with a registered user account.
