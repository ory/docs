...
1. `recovery` - the system sends these messages when users recover their accounts. Used with the `link` recovery strategy.
...
2. `recovery_code` - the system sends these messages when users recover their accounts. Used with the `code` recovery strategy.
   - `recovery_code.valid` - the system sends these messages when the user provided a valid recovery address.
   - `recovery_code.invalid` - the system sends these messages when the recovery flow fails, for example when you start the
     recovery flow using an email address that isn't associated with a registered user account.
3. `verification` - the system sends these messages when users verify their accounts after signing up.
...
4. `verification_code` - the system sends these messages when users verify their accounts after signing up. Used with the `code`
   recovery strategy.
   - `verification_code.valid` - the system sends these messages when the user verifies their account successfully.
   - `verification_code.invalid` - the system sends these messages when the account verification fails, for example when you start
     the verification flow using an email address that isn't associated with a registered user account.
...
To use custom templates in The Ory Network, encode the content of the template with Base64 and use the encoded string to update the Ory Identities config.
Follow these steps:
...
```
  ...
  <TabItem value="selfhosted" label="Self-Hosted Ory Kratos Config"ś>
