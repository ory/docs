...
1. `recovery_code` - the system sends these messages when users recover their accounts, for example after forgetting their
   password.
...
1. `verification` - the system sends these messages when users verify their accounts after signing up.
