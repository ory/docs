...
<Tabs>
  <TabItem value="cloud" label="Ory Network" default>

To use custom templates in Ory Network, encode the content of the template with Base64 and use the encoded string to update the Identity Service config.
Follow these steps:
...
In Ory Network, you can use only Base64-encoded templates. The system doesn't accept `http://` or `file://` URIs. If you provide
...
For security reasons, these Spring functions are disabled in Ory Network:
...
You can use Sprig functions in the nested templates. For security reasons, some functions are disabled in Ory Network.
