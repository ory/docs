...
The variables available for use in email templates depend on the flow and the selected method. The table below lists the most
commonly used fields.

For the complete and current list, see the structs for each template in GitHub under
[`courier/template/email/`](https://github.com/ory/kratos/tree/master/courier/template/email), which is the source of truth.
