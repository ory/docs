...
templates using unsupported methods, the system falls back to using the default templates.
...
Each template must have two versions: `html` and `plaintext`. 
  ...
  - `plaintext` version can't contain any HTML. Must contain only plain text content and any necessary gotmpl logic. This version is used as a fallback when the `html` version cannot be delivered, 
    for example when the user's mail server blocks HTML in all incoming messages.
...
- The `plaintext` version doesn't use any HTML and keeps only the gotmpl logic (`{{ .VerificationURL }}`) and plain text.  
...
as the `Identity` object. This object is a map containing all the attributes of an identity defined in the identity schema, such as `id`, `state`,
...
You can use Sprig functions in the nested templates. For security reasons, some functions are disabled in Ory Cloud. [Click here to 
see the list of disabled functions.](#spring-disabled)
...
This approach is available in self-hosted Ory Kratos instances only. For security reasons, Ory Cloud doesn't allow linking to
external resources. As such, all options must be contained in a single template.

:::

When remote templates are used in self-hosted Ory Kratos, the dynamics of loading nested templates change. Remote templates can't
reference templates outside of the instance's template directory.
