...
Ory Identities sends an email for each template type below. The template type is also the `template_type` field in the
...
The following table is **maintained by hand**. It documents when each email is sent and the condition that gates it. These
semantics are not represented in the source file above.
