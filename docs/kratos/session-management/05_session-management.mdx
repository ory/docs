...
A user can be shown a list of all their active sessions that haven't expired, except the current session using the API from the
...
A user can choose to terminate a session that they no longer need or deem as malicious. The revoke session API is built for this
scenario.
