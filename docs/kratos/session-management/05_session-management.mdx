...
<CodeBlock language="go" title="Initalizing the Go SDK client">{goInitClient}</CodeBlock>
```
...
<CodeBlock language="ts" title="Initalizing the Typescript SDK client">{nativeInitClient}</CodeBlock>
```
