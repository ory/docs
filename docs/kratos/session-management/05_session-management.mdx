...
title: Session management in Ory
...
Users can list their active sessions by sending a `GET` request to the `/sessions` endpoint or by using the `listSessions` SDK method:
