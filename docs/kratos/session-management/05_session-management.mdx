...
You must use a personal access token (PAT) for all administrative session management operations.
...
<CodeBlock language="go" title="Listing all sessions">
{goInitClient}
{goGetSessions}
</CodeBlock>
```
...
<CodeBlock language="go" title="Get a session">
{goInitClient}
{goGetSession}
</CodeBlock>
```
...
<CodeBlock language="go" title="Disabling a session">
{goInitClient}
{goDisableSession}
</CodeBlock>
```
