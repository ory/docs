...
A user can be shown a list of all their active sessions that haven't expired, except the current session using the API from the SDK. This API has a paginated response and optional parameters for page number and number of items per page.
...
[`/sessions/{id}`](../../reference/api.mdx#operation/revokeSessionById) endpoint or use the `revokeSessionById` SDK method:
