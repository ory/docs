...
Ory Elements emits events for form submission actions, including on success, on error, and on validation error.
Your application can listen to these events to perform custom actions in your application.

To attach an event listener, pass a callback function to the form component's corresponding event prop. The available
events are:
...
The `onError` event is emitted when a form submission fails. The reason for the error is exposed via the `error` property of the event.
You can use this event to track a failed form submission and the reason for the failure.
