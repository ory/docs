---
id: overview
title: Ory Elements
sidebar_label: Ory Elements
---

import Mermaid from "@site/src/theme/Mermaid"

# Overview

import WelcomePageSection from "@site/src/components/Welcome/welcome"
import * as welcomeContent from "@site/src/pages/_assets/welcome-content"

<WelcomePageSection {...welcomeContent.identityManagement} />

Ory Elements is a component library that allows you to build custom user interfaces for Ory self-service flows. It provides a set
of pre-built components that can be easily integrated into your application, enabling you to create a seamless user experience
while leveraging Ory's powerful authentication and identity management capabilities.

```mdx-code-block
<Mermaid
  chart={`
flowchart LR
    subgraph ory[Ory Network]
        kratos@{ img: "https://raw.githubusercontent.com/ory/meta/refs/heads/master/static/logos/logo-kratos.svg", pos: "b", w: 120, constraint: "on", label: "Ory Identities" }
        hydra@{ img: "https://raw.githubusercontent.com/ory/meta/refs/heads/master/static/logos/logo-hydra.svg", pos: "b", w: 120, constraint: "on", label: "Ory OAuth2" }
        keto@{ img: "https://raw.githubusercontent.com/ory/meta/refs/heads/master/static/logos/logo-keto.svg", pos: "b", w: 120, constraint: "on", label: "Ory Permissions" }
        polis[Ory Polis]
    end
    subgraph app[Your Application]
        subgraph nextjs[Next.js]
            n@{ img: "/docs/img/examples/nextjs.svg", pos: "b", constraint: "on", h: 44, label: "Next.js" }
            n-->|uses|react
            react@{ img: "/docs/img/examples/react.svg", pos: "b", constraint: "on", h: 44, label: "React" }
            elements@{ img: "/docs/img/logo-elements.svg", pos: "b", w: 120, constraint: "on", label: "Ory Elements" }
            react-->|renders|elements
        end
        subgraph yourCode[Your Code]
            otherCode@{ shape: braces, label: "Your other code" }
            components@{ shape: braces, label: "Custom Components" }
        end

        elements-->kratos
        elements-->hydra

        react-->otherCode
    end
    otherCode-->|calls|ory

    elements-->|renders|components
`}
/>
```

It is designed to work with Ory's self-service flows, such as login, registration, settings, verification, and recovery and the
OAuth2 consent flow.

While it's default experience is a great starting point, Ory Elements allows you to customize the look and feel of your UI to
match your brand and user experience requirements. You can use Ory Elements to build a custom UI that fits your application's
design and user experience, while still leveraging Ory's powerful authentication and identity management capabilities.

Related docs:

- [custom-ui-ory-elements](../kratos/bring-your-own-ui/15_custom-ui-ory-elements.mdx)
- [account-experience](../account-experience/index.mdx)
