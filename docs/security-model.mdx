...
While this is not a common approach that might require a shift in thinking and some getting used to, it's implemented so that you
don't have to worry about refreshing tokens or deciding whether to store them in `localStorage` or `document.cookies`.

Instead, you can devote all your focus and time to developing great software for your users, while Ory does its thing and gives
you best-in-class protection from all common browser attack vectors, such as
...
requirement, HTTP Cookies are ignored by the browser, which prevents the Ory Identity Service (Ory Kratos) from functioning
properly.

Ory exposes the APIs at `https://<project-slug>.projects.oryapis.com`. To manage session information, Ory Identity Service
(Kratos) must be able set the domain in HTTP Cookies to the same domain as the application that consumes its APIs. For example:
...
Ory offers SDKs for certain deployment options such as [Vercel](guides/protect-page-login/nextjs.mdx) which mirror Ory's APIs
...
- Some multi-tenant providers like Heroku (`<your-slug>.herokuapp.com`) or Vercel (`<your-slug>.vercel.app`) expose many apps
  ...
  cookie on the root domain (so `cookie=herokuapp.com`) but only on the subdomain (so `cookie=your-slug.herokuapp.com`).
...
you can currently self-host.

At Ory, we believe that OAuth 2.0 and OpenID Connect isn't a one-size-fits solution. In fact, we think that you probably don't
need to use the complicated protocols at all! As such, we recommend the use of [Ory Hydra](https://github.com/ory/hydra) for
targeted use cases only.
...
[Ory's OAuth2 and OpenID Connect Provider (Ory Hydra)](https://www.ory.sh/hydra) is able to issue OAuth2 Access Tokens in the
JSON Web Token format. Using OAuth2 and OpenID Connect is discouraged unless you have a
[targeted use case for it](https://www.ory.sh/oauth2-openid-connect-do-you-need-use-cases-examples/)

[Ory's Zero Trust Identity and Access Proxy (Ory Oathkeeper)](https://www.ory.sh/oathkeeper) can "convert" Ory Sessions to
JSON Web Tokens. Using Ory Oathkeeper is recommended when developing sophisticated applications with control over network
