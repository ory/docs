...
need to use such complicated protocols at all! We recommend the use of [Ory Hydra](https://github.com/ory/hydra) for targeted use
cases only, such as providing third party integration with your application (e.g. a sign in with [your company] button).
...
[Ory's OAuth2 and OpenID Connect Provider (Ory Hydra)](https://www.ory.sh/hydra) is able to issue OAuth2 Access Tokens in the JSON
Web Token format. Using OAuth2 and OpenID Connect is discouraged unless you have a
[targeted use case for it](https://www.ory.sh/oauth2-openid-connect-do-you-need-use-cases-examples/)

[Ory's Zero Trust Identity and Access Proxy (Ory Oathkeeper)](https://www.ory.sh/oathkeeper) can "convert" Ory Sessions to JSON
Web Tokens. Using Ory Oathkeeper is recommended when developing sophisticated applications with control over network traffic
