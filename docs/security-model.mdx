...
However, your Ory Network project runs on your slug URL `https://{project.slug}.projects.oryapis.com`. The Ory
APIs can't set cookies on a different domain than the one they are running on.

The solution is to tunnel the Ory APIs on a port on your local machine, which allows cookies to be set on the same domain as
...
Do not use the Ory Tunnel in a production environment, because of rate limiting and other issues.
