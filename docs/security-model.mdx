...
However, your Ory Network project runs on your slug URL `https://{project.slug}.projects.oryapis.com`. As mentioned above, the Ory
APIs can't set cookies on a different domain than the one they are running on.

The solution is to "tunnel" the Ory APIs onto a port on your local machine, which allows cookies to be set on the same domain as
the application is running.

Read more on the Ory Tunnel in the [Ory Tunnel documentation](./guides/cli/20_proxy-and-tunnel.mdx#ory-tunnel).

:::note

Do not use the Ory Tunnel in a production environment, as you will run into rate limiting and other issues.
