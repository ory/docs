...
for example a file, network port, physical item, ... . It's up to the
application to map its objects to an unambiguous identifier. The limit on object
identifiers is 64 characters. We recommend the usage of UUIDs as they provide a
high entropy and thus unique identifiers. It's however possible to use for
example URLs or opaque tokens of any kind. Please check the
...
internally, for example a UUIDv4 like `61e75133-efff-4281-8148-a1806919f568` or
...
Because the Keto client can use arbitrary strings as objects, it's tempting to
...
corresponding condition the value fulfills. It's important to understand that
