...
a specific subject (for example user) by some application defined identifier, or
a set of subjects.

## Subject IDs

A subject ID can be any string. It's up to the application to map its users,
devices, ... to a constant, unique identifier. We recommend the usage of UUIDs
as they provide a high entropy. It's however possible to use for example URLs or
...
by defining indirections. They can be used to realize for example
...
of requests it's however required to follow some
...
internally, for example a constant, unique username like `zepatrik` or
preferably UUIDv4 like `480158d4-0031-4412-9453-1bb0cdf76104`.
...
Because the Keto client can use arbitrary strings as subjects, it's tempting to
