...
therefore only be viewed by its owner, `cat lady`. The relation tuple definitions are located in the
...
users that are allowed to view the video. It uses Keto's [expand-API](./concepts/api-overview.mdx#expand-subject-set) to get these
data:
...
Note that there is no direct relation tuple that would grant `cat lady` view access on `/cats/1.mp4` as this is indirectly defined
