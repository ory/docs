...
reference documentation for all details. The expand-API allows to
...
to display who has access to a file and why. In this example, we assume
...
cases the application doesn't want to resolve all subject sets, but rather
wants to display that for example `Everyone in the company` or `Admins` have a specific
relation.
...
The tree doesn't only include the subject IDs (in this case usernames), but
