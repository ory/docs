...
reference documentation for all details. The expand-API allows to expand a given
subject set into all its effective subjects.
...
`files` namespaces respectively. They're identified by a UUID that the
...
to display who has access to a file and why. In this example, we assume that the
application knows the following files and directories:
...
cases the application doesn't want to resolve all subject sets, but rather wants
to display that for example `Everyone in the company` or `Admins` have a
specific relation.
...
The tree doesn't only include the subject IDs (in this case usernames), but also
for what reason they were included. This can be useful for users to audits
