...
resource. This could be done by adding a group, lets call it `hackers`. Now
...
We don't recommend that you cache the responses from Ory Keto. It's designed
to respond quickly and still provide
[some consistency guarantees](../concepts/snaptokens-evaluation-consistency.mdx).
For the revocation of access it's important to not use a local
