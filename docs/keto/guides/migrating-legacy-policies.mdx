...
0.6 release makes Ory Access Control Policy DSL modeled after AWS IAM Policies
obsolete. This guide will help you to rewrite your policies in to
[relation-tuples](https://www.ory.sh/docs/keto/concepts/relation-tuples). You
can read
[The Evolution of Ory Keto: A Global Scale Authorization System](https://www.ory.sh/keto-zanzibar-evolution/)
blogpost to understand a benefits of 0.6 release

## Legacy rules example

The policy below allows `Alice` and `Bob` to create/read/modify/delete
`blog_posts:my-first-blog-post`, `blog_posts:2`, and `blog_posts:3`.
...
According to the example above we need to create required
[namespace](https://www.ory.sh/docs/keto/concepts/namespaces) and
[relation tuple](https://www.ory.sh/docs/keto/concepts/relation-tuples)
...
We need to have `blog_posts` namespace for our example. Let's add the following
content to `keto.yml` configuration file. You can find a good template
...
Let's create an `alice_policies` file with the following content, which adds
...
This example uses the Ory Keto CLI to create the relation tuple using the
[write API](../concepts/api-overview#write-apis)
...
Now, we can use the
[check-API](https://www.ory.sh/docs/keto/guides/simple-access-check-guide) to
verify that `alice` is allowed to `read` the `my-first-blog-post`:
