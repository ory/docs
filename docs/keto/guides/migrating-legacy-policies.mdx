...
- Subjects -> Subject/Subject Sets
- Resources -> Namespaces/Objects
- Actions -> Relations
- Effect -> Became obsolete or can be considered as Relations

We need to have `blog_posts` namespace for our example. Let's add the following content to `keto.yml` configuration file. You can find a good template [here](https://github.com/ory/keto/blob/master/config/keto.yml).
...
This example uses keto cli to create the relation tuple using [write API](../concepts/api-overview#write-apis)
