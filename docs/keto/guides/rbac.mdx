...
The role `admin` inherits all privileges from `author`, which inherits from
`reader`. Only `Alice` (or rather her role `admin`) can delete blog posts,
whereas `author` can create and modify blog posts.
...
groups of users with different access levels. We have the following group of
