...
```
reports:finances#access@(groups:users#member)
groups:users#member@Vincent

reports:pulse_report#access@(groups:management#member)
groups:users#member@(groups:management#member)
groups:management#member@Vincent

db:Settings#create@(db:settings-creator#member)
db:settings-creator#member@(groups:top-management#member)
reports:finance_margin#access@(groups:top-management#member)
groups:management#member@(groups:top-management#member)
groups:top-management#member@Vincent
```
...
curl http://localhost:4466/relation-tuples\?subject_id\=Akyl\&relation\=member

curl http://localhost:4466/relation-tuples\?subject_set.namespace\=groups\&subject_set.relation\=member\&subject_set.object\=users
```

## Check access using expand API
