...
keto relation-tuple create Dilan member groups:community
```
...
The new syntax uses `namespace:object` format (e.g., `reports:finance`). The old syntax `namespace object` (e.g., `reports finance`) is still supported but deprecated and will show a warning message.
...
You can create this relation tuple using:

```bash
keto relation-tuple create Dilan member groups:marketing
```
