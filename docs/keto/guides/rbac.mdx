...
`marketing`, `finance` and `admin`:

The role `admin` inherits all privileges from `finance`, `marketing` and
`community`
...
attributes (e.g. `Dilan` needs to have access only during work hours), or in
...
- There is no concept of ownership: _Dan is the author of article "Hello World"
