...
[Ory Permission Language](../reference/ory-permission-language) (OPL). You can then run fine-grained checks against Ory Keto,
which are answered based on a combination of OPL and the concrete high-level relationships stored.
...
After logging in, navigate to `Namespaces & Rules` in the sidebar, and to the `Permisson Rules` tab.
...
### Connecting to the Ory Permission Language via CLI
...
If you do not want to use the Ory Console you need to [install Keto](../install.mdx) locally. Next, create a `keto.yaml` file with
this basic content:
