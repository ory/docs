...
reach `user`. The request is allowed if there is such a path.
...
Because of this we decided that generic benchmarks won't yield any meaningful
...
performance considerations apply here, while it's important to note that
