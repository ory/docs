...
where each `owner` of an object also has `access` to that object. All relationships are stored in that namespace.
...
[relationship](../concepts/relation-tuples.mdx) to Ory Keto through the [write-API](../concepts/api-overview.mdx#write-apis):
...
a [write-API request](../concepts/api-overview.mdx#write-apis) adding the following relationship to Ory Keto:
