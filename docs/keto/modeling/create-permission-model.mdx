...
In Ory Permissions, relationships are declared inside the corresponding class in the Ory Permission Language. Since you store
arbitrary tuples in Ory Permissions, you always have many-to-many relations between these objects. To reflect this in OPL, the
convention is to pluralize the relation name, for example, `viewers` instead of `viewer`.
