...
This command starts an HTTP tunnel to Ory on the domain `[tunnel-url]` you specify. This
...
on your machine. It is similar to the `ory proxy` command, but it does not require to
...
In contrast to the `ory proxy`, the tunnel does not alter the HTTP headers arriving at your
application and it does not generate any JSON Web Tokens.
