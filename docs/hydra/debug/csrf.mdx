...
Because Ory OAuth2 & OpenID Connect isn't just an API, but instead talks to your users' browsers directly, several security
measures have been implemented in Ory Identities. One of them is protection against CSRF:
...
![Google Chrome Developer Tools - Application Tab - Cookies](../_static/csrf_app_tab.png)

as well as the network tab - look for `Cookie` and `Set-Cookie` HTTP Headers:

![Google Chrome Developer Tools - Network Tab - Cookies](../_static/csrf_network_tab.png)
