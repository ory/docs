...
Sometimes, calling `/oauth2/sessions/logout` doesn't behave as expected, for
example:
...
- You are mixing up domain names (for example `127.0.0.1` and `localhost` -
...
`oauth2_authentication_session` for the URL Ory Hydra is running on, and for the
domain that's in your `http://.../oauth2/sessions/logout`**
