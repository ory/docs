...
up. Let's call the client `facebook-photo-backup`. We've to specify which
...
- `--scope openid,offline,photos.read` allows the client to request various
  ...
  - `photos.read` this is an imaginary scope that'sn't handled by Ory Hydra
...
Just imagine this being, for example, passport.js that's generating an auth
