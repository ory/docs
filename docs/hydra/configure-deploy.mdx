...
[configuration file](reference/configuration.mdx) instead, remember to mount it in the container. For example:
...
called `hydra token user`. Just imagine this being, for example, passport.js that's generating an auth code url, redirecting the
