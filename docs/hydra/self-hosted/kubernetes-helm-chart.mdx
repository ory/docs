...
To install Ory Hydra, the following [configuration values](../reference/configuration) must be set:
...
You can pass your [Ory Hydra configuration file](../reference/configuration) by creating a yaml file with key `hydra.config`
...
clients using the `oauth2clients.hydra.ory.com` custom resource. By default, this component is enabled and installed together with
