...
This example shows a basic configuration of Ory Hydra on a single virtual machine (VM). The configuration of a VM may vary depending on
...
use the following script to install Node.js 16 on a Ubuntu system.
...
5. Add configuration for Ory Hydra in `hydra.yml` to use the Postgres database.  
   Open `hydra.yml` and change the DSN configuration. Ory Hydra is storing data in an SQLite database in the default quickstart configuration.
   Change the DSN key to use the Postgres database you configured before and add the `issuer` URL. This URL will be used as `issuer` in access and ID tokens.
