...
This guide explains how to setup and run Ory Hydra in an exemplary production environment. It uses Postgres as database, Nginx as
...
This example shows basic configuration of Ory Hydra on a single virtual machine (VM). The configuration of a VM may vary depending on
...
Since the default version of [Node.js](https://nodejs.org/en/) is outdated we need to install a newer version. You can
...
3. Create a Postgres user for Hydra:

```bash
CREATE USER hydra PASSWORD '<YOUR_PASSWORD_HERE>';
```

4. Edit the `/etc/postgresql/12/main/pg_hba.conf` file and add the following to enable `scram-sha-256` encryption:

```bash
host    all             all             127.0.0.1/32            scram-sha-256
```

5. Check your credentials against Postgres:
...
6. Use the password for the Ory Hydra user we created before to log into Postgres. If everything works correctly you should see a prompt similar
   to this:
...
Congratulations, you have installed and configured the PostgreSQL database. Next we will set up Ory Hydra.
...
wget https://github.com/ory/hydra/releases/download/${useLatestRelease(
  "hydra"
)}/hydra_${useLatestRelease("hydra")}-linux_64bit.tar.gz
# Extract contents
...
5. Add configuration for Ory Hydra in `hydra.yml` to use the Postgres database.  
   Open `hydra.yml` and change the dsn configuration. In the default quickstart configuration Ory Hydra is storing data in an SQLite
   database. Change the DSN key to use the Postgres database you configured before and add the `issuer` URL. This URL will be used as
   `issuer` in access and ID tokens.
...
4. Configure SSL via Certbot. Run the following command and answer the questions to set it up. When prompted choose to redirect HTTP
   traffic to HTTPS.
...
There are some parts missing in your configuration at this point, and we need to configure locations and
...
5. Add the following configuration before `server` section to the `/etc/nginx/sites-enabled/oauth2.example.com` file:
...
- Fork the [Ory Hydra NodeJS UI Reference](https://github.com/ory/hydra-login-consent-node) or
