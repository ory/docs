...
Let's create a user for Hydra (Use your own password / hash!):

```bash
CREATE USER hydra PASSWORD 'CHANGE-ME-INSECURE-PASSWORD';
```

We need to change the `/etc/postgresql/12/main/pg_hba.conf` file and append the following to enable `scram-sha-256` encryption:
...
Type or paste the password for the Ory Hydra user we created before. If everything works correctly you should see a prompt similar
to this:
...
We need to add some configuration in `hydra.yml` to use our Postgres We need configure Ory Hydra to use the Postgres database.
...
Test your setup using the [`serve` command](../cli/hydra-serve):
...
We need to change the owner of `/opt/hydra` directory to the `hydra` user:
...
Paste the following content to configure systemd to start Ory Hydra using the [`serve` command](../cli/hydra-serve) from earlier:
...
Add the following configuration before `server` section to the `/etc/nginx/sites-enabled/oauth2.example.com` file:
...
You can test if Ory Hydra runs by doing a health check at
