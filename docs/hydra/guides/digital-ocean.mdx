...
This guide explains how to setup and run Hydra in a
...
This guide uses oauth2.example.com as a hostname to run Hydra.
...
```bash
CREATE USER hydra with password 'SCRAM-SHA-256$4096:N66uuf4gEDEdvqhrInf7tw==$mnm9XgQuRIA3z8DlLa+E710GewLIDGpj2+hKyhiHOqM=:ypiPv/R1nnVc6yaiyektFuA4wnqa0xX4qjRNFKdwU2g=';
CREATE ROLE
...
Type "help" for help.
```
We installed and configured PostgreSQL database and let's continue with Hydra setup
## Installing Hydra
...
At this point, we need to make additional configuration steps. Let's change some configuration parameters in `hydra.yml` file
...
```
## Running Hydra using Systemd
We need to change ownker of `/opt/hydra` directory to `hydra` user
```bash
chown -R hydra /opt/hydra/
```
Let's create a `/etc/systemd/system/hydra.service` file with the following content
```
[Unit]
...
We have Hydra up and running and listening loopback interface
...
Let's create a default configuration for the virtual host. Let's create a `/etc/nginx/sites-avaiable/oauth2.example.com` with the following content
...
One can balance network traffic between different instances of Hydra running
...
Let's add the following configuration before `server` section to `/etc/nginx/sites-enabled/oauth2.example.com` file
```
upstream public_api {
...
...
```
Let's add our locations and the `/etc/nginx/sites-enabled/oauth2.example.com` has the following content
```
upstream public_api {
        server 127.0.0.1:4444;
        server 127.0.0.1:4444; # We can load balance the traffic to support scaling
