...
This guide explains how to setup and run Ory Hydra in a
...
This guide uses `oauth2.example.com` as a hostname to run Ory Hydra. Replace it with `<something>.<your-domain>`.
...
Now we can create a user by running the following command, fill in the password hash you copied in the step before:
...
Type "help" for help.
```
We installed and configured the PostgreSQL database. Let's continue with the Ory Hydra setup
...
We need a default configuration for the virtual host. Let's create a `/etc/nginx/sites-avaiable/oauth2.example.com` with the following content:
...
We have missing parts in our configuration, and we need to configure locations and [upstream APIs](http://nginx.org/en/docs/http/ngx_http_upstream_module.html).
...
Add the following configuration before `server` section to `/etc/nginx/sites-enabled/oauth2.example.com` file:
