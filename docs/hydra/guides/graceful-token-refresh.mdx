...
- If `rotation_grace_reuse_count` is set to a positive integer (e.g., 3), a refresh token can only be reused up to that many times
  within the grace period. After the limit is reached, further attempts to use the token will be rejected.
- If `rotation_grace_reuse_count` is set to `0` (the default), there is no limit on the number of refreshes within the grace
  period.
