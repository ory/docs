...
The Hydra `config.yaml` file located in the main Hydra directory allows you to adjust:
...
[OAuth2ClientLifespans](../reference/api#operation/UpdateOAuth2ClientLifespans) endpoint of the Hydra admin API. The client
...
You can change the configuration on the instance-wide level by adjusting specific keys in the Hydra configuration.
...
```shell
$ curl -X PUT localhost:4445/clients/db779000-01f3-4c12-bf70-ff85933d485e/lifespans --data '{
