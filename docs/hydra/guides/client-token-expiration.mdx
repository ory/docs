...
perform the "login flow" again when performing the OAuth2 Authorization Code Flow or OpenID Connect Implicit / Hybrid Flow.

Furthermore, OpenID Connect Back-Channel Logout will not work if the authentication session is expired which is a known limitation
