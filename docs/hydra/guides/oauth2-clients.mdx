...
remains valid until you remove it, allowing you to update all your client services without service interruption.

##### How secret rotation works

1. Generate a new secret for the client service. Both the old and the new secrets can be used to authenticate from this point on.
...
dropped and can no longer authenticate. Setting a new secret through the update or patch client APIs clears all rotated secrets.
...
The response includes the new `client_secret`. Ory shows this value only once. Store it immediately in the secret store that your
client services read from, such as a secrets manager or an encrypted environment configuration.
...
Secret rotation enables zero-downtime credential updates. Both the old and new secrets remain valid until you manually remove the
old secret, allowing you to update all your client services without service interruption.
