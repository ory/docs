...
The `/userinfo` endpoint returns information on a user given an access token. Since Ory Hydra is agnostic to any end-user data,
the `/userinfo` endpoint returns only minimal information per default:
