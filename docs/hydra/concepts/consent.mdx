...
Ory OAuth2 & OpenID doesn't contain a database with End-Users but instead uses HTTP Redirection to "delegate" the login and
consent flow to another app - we call this the Login & Consent App.
...
Once the login challenge is accepted, Ory OAuth2 & OpenID will ask the user for consent:

![OAuth 2.0 Asking for User Consent](../images/google-consent-playground.png)

The UI is fully under your control, because Ory OAuth2 & OpenID redirects the End-User's browser to the "Consent Endpoint"
...
Ory OAuth2 & OpenID appends a `consent_challenge` query parameter to the url. The value is a ID which should later be used by the
Consent Endpoint to fetch important information about the request.
...
Check the ["Implementing the Consent Endpoint" Guide](../guides/consent.mdx) for examples using the Ory OAuth2 & OpenID SDK in
different languages.
...
```json
{
  ...
  // Remember, if set to true, tells Ory OAuth2 & OpenID to remember this consent authorization and reuse it if the same client
