...
which is why you probably ended up at Ory OAuth2 & OpenID Connect. But before you start hacking away, the best use of your time
...
more, check out [Ory Identities](https://www.ory.sh/kratos) first!
...
your user's data, Ory OAuth2 & OpenID Connect is the right tool for you.
...
in with Google", "Sign in with Apple"), Ory OAuth2 & OpenID Connect is the right tool for you.
...
with OAuth2 right away. It's recommended to use [Ory Identities](https://www.ory.sh/kratos) for your basic user management needs
and then figure out if you still need OAuth2.
...
OAuth2 or OpenID Connect right now. Instead, you should give [Ory Identities](https://www.ory.sh/kratos/) a spin, which aims to
solve that.
...
- Ory OAuth2 & OpenID Connect maintains a session cookie. When a user signs in, the cookie will be set for that user. This allows
  the next OAuth2 request to complete without requesting the user to sign in again.
...
1. you realize that you only want to solve sign up, login, session management and that you don't need OAuth2 - your next step in
   this case should be to check out [Ory Identities](https://www.ory.sh/kratos/) which does solve all of that;
2. you are reaffirmed that you need OAuth2 and OpenID connect because you want developers you don't know to interact with your
   users and your users' data - in which case Ory OAuth2 & OpenID Connect is a great fit for you!
...
all software that use OAuth2 and OpenID Connect as their primary protocols.

If all you need is user registration, log in, user settings, and similar flows - check out
[Ory Identities](https://www.ory.sh/kratos/)!
...
one HTTP request using the access token. This is mainly true for systems that have refresh tokens which are valid for one use
only.
...
[Ory Identities](https://www.ory.sh/kratos/) works both with tokens and cookies. Implementing login doesn't involve any manual
token handling - just log in and you're done!
