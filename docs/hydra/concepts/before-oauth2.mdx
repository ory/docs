...
potential sign in (for example be listed alongside "Sign in with Google", "Sign in with
...
that - for example YouTube, which runs on a separate domain - doesn't use
...
- Complex integration with Social Sign In (for example Twitter, Facebook, ...) on
  native mobile apps (see for example
...
OpenID Connect well. These limitations apply to all software (for example Ory Hydra,
...
access token. This is mainly true for systems that have refresh tokens which
are valid for one use only (such as Ory Hydra, Auth0, ...).

**Cookies are great** and a common misconception is that cookies are less secure
than tokens or less usable. For 95% of web applications it's enough to work
with cookies, if they only run in the browser and not in a native
