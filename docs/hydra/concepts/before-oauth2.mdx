...
which is why you probably ended up at Ory OAuth2 & OpenID Connect. But before you start hacking away, the best use of your time right now
is to re-evaluate if you really need OAuth2 and OpenID Connect.
...
- Ory OAuth2 & OpenID Connect maintains a session cookie. When a user signs in, the cookie will be set for that user. This allows the next
