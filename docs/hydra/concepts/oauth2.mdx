...
This section describes on a high level what OAuth 2.0 is and how it works. Please keep in mind that Ory OAuth2 & OpenID Connect is
a technology targeted at professionals using OAuth 2.0 on a regular basis. While we try to cover some of the concepts, OAuth 2.0
and OpenID knowledge is expected from you in order to understand and use Ory OAuth2 & OpenID Connect to its fullest potential.
...
token after someone logs in", take a look at [Ory Identities](https://www.ory.sh/kratos) instead!
...
1. A developer registers an OAuth 2.0 Client (external application) with the Authorization Server (Ory OAuth2 & OpenID Connect)
   the intention to obtain information on behalf of a user.
...
The usage of an access token or client credentials is required to access the endpoint. Ory OAuth2 & OpenID Connect will however
accept any valid token or valid credentials as there is no built-in access control.

## Token Formats

Ory issues access tokens, refresh tokens, and authorization codes with a fixed prefix:

* `ory_at_`: OAuth 2.0 Access Token
* `ory_rt_`: OAuth 2.0 Refresh Token
* `ory_ac_`: OAuth 2.0 Authorization Code

These help identify OAuth2 credentials in code scanning tools.
