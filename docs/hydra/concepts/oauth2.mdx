...
This section describes on a high level what OAuth 2.0 is and how it works. Please keep in mind that Ory OAuth2 & OpenID is a
technology targeted at professionals using OAuth 2.0 on a regular basis. While we try to cover some of the concepts, OAuth 2.0 and
OpenID knowledge is expected from you in order to understand and use Ory OAuth2 & OpenID to its fullest potential.
...
1. A developer registers an OAuth 2.0 Client (external application) with the Authorization Server (Ory OAuth2 & OpenID) the
   intention to obtain information on behalf of a user.
...
Ory OAuth2 & OpenID has pre-defined OAuth 2.0 Scope values:
...
The usage of an access token or client credentials is required to access the endpoint. Ory OAuth2 & OpenID will however accept any
valid token or valid credentials as there is no built-in access control.
