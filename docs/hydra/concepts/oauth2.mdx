...
- OpenID Hybrid Flow with Authorization Code Flow reponse type: `response_type=code+{other_response}`
...
When [accepting consent requests](consent), the `offline_access` scope must be included in the
list of `grant_scope`:
...
You can adjust the Refresh Token lifespan (TTL) using the `ttl.refresh_token` configuration key. By default, 
