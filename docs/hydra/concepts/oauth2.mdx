...
3. The **resource provider** is the service that hosts (provides) the resources.
   These resources (for example blog articles, printers, todo lists) are owned by a
...
When performing an OAuth 2.0 Flow where the end-user is involved (for example Implicit
...
using the `grant_access_token_audience` key. In most cases, it's ok to grant
