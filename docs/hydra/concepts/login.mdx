...
Flow", "Implicit Flow", ...) you want to use some of the query parameters (for example
...
`false`, the user interface must be shown. If `skip` is true, you shouldn't
...
challenge, or you **reject** (for example the user isn't allowed to perform OAuth2
...
This endpoint isn't compatible with OpenID Connect Front-/Backchannel logout
and doesn't revoke any tokens.
