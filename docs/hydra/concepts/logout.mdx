...
- `***`: Here, the "valid session cookies" implies that the browser has a valid
  ...
  not mix up IP (for example `127.0.0.1`, `192.168.1.1`) addresses and FQDNs (for example
...
provider (for example `https://my-logout-provider/logout?challenge=...`). Next, the
...
action such as confirming the logout request. It's RECOMMENDED to request
...
user-agent (such as Browser) but from Ory Hydra directly, the session cookie of the
end-user won't be available to the OAuth 2.0 Client and the session has to be
