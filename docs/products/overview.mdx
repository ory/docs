...
standard-based OIDC tokens (access, refresh, ID tokens).
7. The client sends the access token on requests to your backend/API (resource server).
8. The backend validates the access token with Ory Hydra (via introspection or JWT validation). Ory Hydra detects 
the 'openid' scope was requested and granted, and responds with an ID token in addition to the access token.
Scopes in token govern what is allowed.
...
external federated authentication and you have to bridge legacy SAML-based client app with modern OIDC-base IdPs.
...
new OIDC response to send back to the client app.
