...
Integrate with social sign-in providers may require proving the ownership of the authorized redirect URI domain. If that is the
case, you must use a custom domain and the `https://{project.slug}.projects.oryapis.com` URL can't be used as the redirect URI.
...
`base_redirect_uri` configuration. To do that, use Ory CLI:
...
To understand how to fix the problem, let's look at how Ory handles the OIDC registration flow. There are 5 the system performs
users register with a social sign-in provider:
