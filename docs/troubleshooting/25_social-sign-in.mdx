...
There are two cases where Ory will redirect back to the registration page after a successful OIDC flow:

1. Some required traits couldn't be mapped from the Social Provider and need manual input from the user, for example a phone
   number.
2. The Jsonnet configuration for the Social Provider is incorrect and cannot map the data correctly.

In this section, we will focus on the second case.
...
4. Ory maps the user information to the [Identity Schema traits](../manage-identities/identity-schema)
5. (optional) Ory redirects back to the registration page with the missing traits filled in

When step 4 fails, Ory will be stuck in a redirect loop as shown below.

![Registration page with only a `Continue` button](./_static/registration-page-with-continue-button.png)

Since the Identity Schema can be customized, Ory relies on Jsonnet to map the retrieved information to the Identity Schema traits.
Please take a look at [Social Sign-in data mapping](../kratos/social-signin/data-mapping) for further information.
