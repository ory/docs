...
- **Scenario 2** - Migrating from Auth0 or another OAuth2-based solution
  ...
  to the [Ory OAuth2 quickstart guide](/docs/getting-started/oauth2-openid/add-oauth2-openid-connect-nodejs-expressjs) which provides
...
1. Establish a connection between the Ory identity and the user record in your database by storing the `user.id` in
   ...
   user. More about metadata in the [Identity metadata & traits ](/docs/kratos/manage-identities/managing-users-identities-metadata)
   documentation.
...
[create identity API](/docs/reference/api#tag/identity/operation/createIdentity) to import your users. If the hashing algorithm isn't
...
You can use the [create identity API](/docs/reference/api#tag/identity/operation/batchPatchIdentities) to bulk import identities into
