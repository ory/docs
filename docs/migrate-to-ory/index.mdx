...
define identity schema integrate into FE services (incl. UI) integrate into BE services export identities with credentials
transform identities (CSV -> JSON, rewrite keys, ...) import identities go live
...
- send them to the project creation screen in the console?

- they should first migrate your dev or staging project before doing it in production.

We recommend you try the migration using dev / stage project
...
use only the components you need and style them to match your implementation's design. The UI created with Ory Elements changes
