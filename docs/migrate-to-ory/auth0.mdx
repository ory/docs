...
You must be on a paid Auth0 plan to export password hashes. Auth0 makes no commitments about the processing time for this kind of
request.
...
used before the migration. If you can't get users' password hashes, you can still import Auth0 user accounts to Ory. When password
hashes are not imported, users you migrated must create a new password when they log in for the first time after the migration.
...
3. Choose **I have a question regarding my Auth0 account** and pick the **I would like to obtain an export of my tenant password
   hashes** option.
4. Fill in the subject and description and submit the ticket.
...
the script [here](https://raw.githubusercontent.com/ory/docs/auth0guide/code-examples/migrate-to-ory/0-get-auth0-user-data.sh).

### Get the API Access Token and connection ID
...
2. Select **Auth0 Management API** and go to the **API Explorer** tab. Copy the displayed token.
   ...
   The token is valid for 24 hours. When this time passes, repeat a process to generate a new token.
...
The script accounts for all possible metrics you can export in a bulk user export. The process takes some time to complete and
will be downloaded when the results are ready. The bulk user export is a compressed, newline-delimited JSON.
...
- If you import the Auth0 user data from the bulk user export and the password hashes, your users can log in with their emails and
  passwords.
- If you can't get password hashes from Auth0, create new users for the known email addresses and the associated data. In this
...
[custom script](https://raw.githubusercontent.com/ory/docs/auth0guide/code-examples/migrate-to-ory/1-create-ory-identities.sh). To
use it, you must install:
