...
- Press `Enter` or hit the `Update` button to save it, once you have made your
  changes. Currently, it is not possible to edit an existing Identity Model on the same revision. The identity model can still be reused as a template for a following revision.
...
- Select the "Customer Type 1" Identity Model and press `Customize Identity Schema`.
