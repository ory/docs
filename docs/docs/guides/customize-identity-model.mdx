...
- When finished editign name your custom Identity Model. Input the name in the
  textbox between the checkbox and the schema.

- Press `Enter` or hit the `Update` button to save it, once you have made your
  changes. Currently it is not possible to edit an exisiting Identity Model, so
...
in the properties keyword. This ususally should be set to false.
...
Please not that this is just a reference Identity Model, for practical uses it
