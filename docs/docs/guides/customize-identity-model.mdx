...
- Press `Enter` or hit the `Update` button to save it, once you have made your
  changes. Currently, it is not possible to edit an existing Identity Model,Currently, it is not possible to edit an existing Identity Model on the same revision. The identity model can still be reused as a template for a following revision.
...
![Email Password Identity Model](../images/identity-model/email-password.png)

The part highlighted below defines the identities `email` for the
email+password flow in the Ory Cloud. It also includes a method for
...
Currently, only the `email` method is available (recovery/verification via a
...
### Changing existing Identity Models

While it is currently not possible to directly edit existing Identity Models, you can make revisions.
For example, an Identity Model named "Customer Type 1" exists and you would like to make changes to it:

- Select the "Customer Type 1" Identity Model and hit `Customize Identity Schema`.
- Make the necessary changes.
- Enter a new name, e.g. "Customer Type 2".
- Press `Enter` or `Update` to save it.
...
in the properties keyword. This currently has no effect and should be set to false.
...
fields for the users' age. There are also two true/false fields for specifying
the newsletter subscription and enterprise status.
