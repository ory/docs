...
![Custom Domain Creation form](./custom-domains/cname-create.png)
...
provider with which the domain is registered, with the value that is provided in
...
status to see if it was configured correctly with your registrar. Note that even
if you have done everything correctly, it can take up to 24 hours for your
...
![Check custom domain details](./custom-domains/cname-check.png)

When the domain is reachable, it will have the "Active status".

Once active the Ory Managed UI is reachable at e.g. `https://your.custom-domain/ui/welcome`.  
Learn how to configure a custom UI: [Bring Your Own UI](bring-your-user-interface.mdx).
