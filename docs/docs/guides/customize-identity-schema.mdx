...
Ory supports custom Identity Schemas. To learn more about the concept of Identity
Models at Ory, please head over to
[Identity Data Model, Custom Fields](../concepts/identity.mdx). The Identity
Model is a JSON Schema which describes the traits that make up an identity, for
...
- Head over to the "Customize / General" tab (URL:
  ...
  currently active Identity Schema.
...
Identity Schema more accurately, e.g.:
...
For example, an Identity Schema named "Customer Type 1" exists and you would like
...
The following Identity Schema includes first/last and nickname, as well as number
...
Identity Schema. Ideally, the Identity Schema should only include traits that are
