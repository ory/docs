...
description:
  ory proxy production Run an application in production mode with Ory
  integration
---
...
application. This command works for remote environments, for example when
deploying a React, NodeJS, Java, PHP, ... app to a server / the cloud.

    $ ory proxy production --port 4000 \
    	http://localhost:3000 \
    	example.org

If you want to expose the application / proxy at a specific port, append the
port to the domain name:
