...
The `Post-Recovery` redirect is currently not supported. The current
...
On the sub-conditions such as `Post-Password` or `Post-OIDC` a FQDN or a path
...
As mentioned above, the `Post-Recovery` redirect is currently not supported and
will automatically redirect the user to the `Settings Ui` url. This implies that
you should currently rely on the `Post-Settings` redirect url for
`Post-Recovery` flows as well.
