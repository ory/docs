...
e.g. a path such as `/login` will alter https://domain.example/global/path
...
the default value which is the Ory Cloud [Managed User Interface](./managed-ui.mdx).

Once a flow has been reset or updated it will take effect immediately on **new** flows.
Old flows that have not expired yet will not be updated.
