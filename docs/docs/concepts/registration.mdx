...
The Registration Form payloads are then submitted to Ory which responds with:
...
To finish the Registration Flow, the end-user fills out the Registration Form.

The Registration Form must include a field marked as the identifier in the Identity JSON
Schema, for example:
...
A successful registration is different for Browser Clients and API Clients. 
