  ...
  return (
    <Tabs>
      <TabItem value="macos" label="macOS" default>
        <CodeBlock language="shell">{`${hint}export ORY_PROJECT_SLUG=${url}`}</CodeBlock>
      </TabItem>
      <TabItem value="linux" label="Linux">
        <CodeBlock language="shell">{`${hint}export ORY_PROJECT_SLUG=${url}`}</CodeBlock>
      </TabItem>
      <TabItem value="banana" label="Windows CMD">
        <CodeBlock language="shell">{`${hint}set ORY_PROJECT_SLUG=${url}`}</CodeBlock>
      </TabItem>
      <TabItem value="powershell" label="Windows Powershell">
        <CodeBlock language="powershell">{`${hint}$Env:ORY_PROJECT_SLUG = "${url}"`}</CodeBlock>
      </TabItem>
...
docker-compose -f quickstart.yml -f contrib/quickstart/kratos/cloud/quickstart.yml up --build --force-recreate -d`}</CodeBlock>
        ...
        <CodeBlock language="shell">{`export ORY_PROJECT_SLUG=http://localhost:4433`}</CodeBlock>
      </TabItem>
    </Tabs>
  )
}
