...
- [reverse proxy](@site/docs/oathkeeper/index.mdx#reverse-proxy)
- REST API which serves the
  [Access Control Decision API](@site/docs/oathkeeper/index.mdx#access-control-decision-api) as
  well as other API endpoints such as health checks, JSON Web Key Sets, and a
  list of available rules.

For this guide we're using Docker. Ory Oathkeeper however can be
[installed in a variety of ways](@site/docs/oathkeeper/install.mdx).
...
[configuration chapter](@site/docs/oathkeeper/reference/configuration.mdx).
...
We encourage you to check out our [helm charts](https://k8s.ory.com/helm/) which
