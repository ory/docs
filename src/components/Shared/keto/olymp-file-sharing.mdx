...
This example assumes there is a [namespace](/docs/keto/concepts/namespaces) `files` with the relations `owner` and `access`
...
[relationship](/docs/keto/concepts/relation-tuples) to Ory Keto through the
[write-API](/docs/keto/concepts/api-overview#write-apis):
...
application will internally request all [objects](/docs/keto/concepts/objects) (file IDs) with the owner `demeter` by using the
[list-API](/docs/keto/concepts/api-overview#list-relationships). The response will contain the object
...
a [write-API request](/docs/keto/concepts/api-overview#write-apis) adding the following relationship to Ory Keto:
...
[expand-API](/docs/keto/concepts/api-overview#expand-subject-sets) to compile a list of everyone who can access the file:
...
[check-API](/docs/keto/concepts/api-overview#check-relationships) to verify that `athena` has access to the file before it returns
