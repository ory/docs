...
This example assumes there is a [namespace](@site/docs/keto/concepts/05_namespaces.mdx)
`files` with the relations `owner` and `access` defined, where each `owner` of
an object also has `access` to that object. All relationships are stored in that
namespace.
...
[relationship](@site/docs/keto/concepts/01_relation-tuples.mdx) to Ory Keto through the
...
[objects](@site/docs/keto/concepts/10_objects.mdx) (file IDs) with the owner `demeter` by
using the [list-API](@site/docs/keto/concepts/25_api-overview.mdx#list-relationships). The
...
[write-API request](@site/docs/keto/concepts/25_api-overview.mdx#write-apis) adding the
following relationship to Ory Keto:
...
[expand-API](@site/docs/keto/concepts/25_api-overview.mdx#expand-subject-sets) to compile a
list of everyone who can access the file:
...
uses the [check-API](@site/docs/keto/concepts/25_api-overview.mdx#check-relationships) to
