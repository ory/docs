...
product that generates such an attestation on the Simulator; its code compiles to nothing in device builds, so it can never ship
in production. Relaxed attestation is refused once the project leaves the development environment.
