...
4. Store the sealed blob together with the salt, KDF parameters, IV, a format version, and the key alias. On iOS use the Keychain
   ...
   migrations. Keychain items survive app deletion, so detect a reinstall — for example with a `UserDefaults` install marker,
   which the system clears on deletion — and drop any stale artifacts before use. Android Keystore keys are purged on uninstall
   automatically, along with their hardware-backed sealing key.
