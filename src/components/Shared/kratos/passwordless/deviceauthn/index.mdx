...
- <SameDeploymentLink to="kratos/passwordless/deviceauthn/android">
    Device binding on Android
  </SameDeploymentLink>
- <SameDeploymentLink to="kratos/passwordless/deviceauthn/ios">
    Device binding on iOS
  </SameDeploymentLink>
- <SameDeploymentLink to="kratos/passwordless/deviceauthn/flutter">
...
- `ios_app_ids` / `android_app_ids` — allow-lists checked against the
  attestation. Use the Apple App ID (`<TeamID>.<BundleID>`) and the SHA-256
...
`{"nonce":"<base64 of 32 raw bytes>"}`. Decode twice to obtain the raw nonce
