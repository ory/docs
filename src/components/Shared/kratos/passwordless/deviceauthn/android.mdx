...
For both factors, pin enrollment to your app: set `android_app_ids` to the lowercase-hex SHA-256 digest of your app signing
certificate, so that only keys created by your app can be enrolled. Get the digest with `keytool -printcert -jarfile app.apk` (or
the Gradle `signingReport` task) and strip the colons:
...
selfservice:
  methods:
    deviceauthn:
      config:
        android_app_ids:
          - "3d8f2a6c1b7e4d90a5c8e2f4b6d0a19c7e3f5b8d2a4c6e0f9b1d3a5c7e9f0b2d"
```
...
- Create the signing key **with** `setUserAuthenticationRequired(true)` and enroll it with `"user_verification": "platform"` and
  ...
  <SameDeploymentLink to="kratos/passwordless/deviceauthn#biometric-enrollment">Biometric enrollment</SameDeploymentLink>).
...
- See <SameDeploymentLink to="kratos/passwordless/deviceauthn#protocol-reference">Protocol reference</SameDeploymentLink> for the
  flow each ceremony maps to; the JSON request and response bodies live there, linked below.

The code above zeroizes every PIN, derived key, and secret in a `finally` block and never persists the transport key — keep that
discipline when you wire these calls.
...
3. **Rotate** (<SameDeploymentLink to="kratos/passwordless/deviceauthn#rotating-the-pin-secret">Rotating the PIN
   secret</SameDeploymentLink>) — a fresh `PinCeremony`, `sign(alias, nonce + transportPublicKey)` over the unhashed
   concatenation, submit the `rotate_secret` payload, then open and re-seal exactly as at enrollment.
...
server-side to accept the signing key. Relaxed-attestation keys expire after 30 days, so re-enroll after that.

- See <SameDeploymentLink to="kratos/passwordless/deviceauthn#relaxed-attestation-for-testing">Relaxed attestation for
  testing</SameDeploymentLink> for the details.
