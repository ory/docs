...
Read <SameDeploymentLink to="kratos/passwordless/deviceauthn">Client
implementation requirements</SameDeploymentLink> together with this section —
the comments in the code below are normative and restate those rules inline.
...
- Create the signing key **with** `setUserAuthenticationRequired(true)` and
  ...
  <SameDeploymentLink to="kratos/passwordless/deviceauthn">
...
See <SameDeploymentLink to="kratos/passwordless/deviceauthn">Protocol
reference</SameDeploymentLink> for the flow each ceremony maps to; the JSON
request and response bodies live there, linked below. The code above zeroizes
every PIN, derived key, and secret in a `finally` block and never persists the
transport key — keep that discipline when you wire these calls.

1. **Enroll** (<SameDeploymentLink to="kratos/passwordless/deviceauthn">PIN
   enrollment</SameDeploymentLink>) — `decodeNonce` the flow's
...
2. **Log in**
   (<SameDeploymentLink to="kratos/passwordless/deviceauthn">First-factor
   login</SameDeploymentLink>) — `decodeNonce`, `PinVault.unseal` with the
...
3. **Rotate** (<SameDeploymentLink to="kratos/passwordless/deviceauthn">Rotating
   the PIN secret</SameDeploymentLink>) — a fresh `PinCeremony`,
...
See <SameDeploymentLink to="kratos/passwordless/deviceauthn">Relaxed attestation
for testing</SameDeploymentLink> to learn more.
