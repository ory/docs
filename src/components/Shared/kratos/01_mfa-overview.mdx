...
[step-up authentication](@site/docs/kratos/mfa/05_step-up-authentication.mdx) document.
...
native apps. Read [Time-based one-time passwords (TOTP)](@site/docs/kratos/mfa/15_totp.mdx)
to learn more.
...
[WebAuthn and FIDO2 (YubiKey)](@site/docs/kratos/mfa/20_webauthn-fido-yubikey.mdx) to learn
more.
...
[Lookup Secrets (Recovery Codes)](@site/docs/kratos/mfa/25_lookup-secrets.mdx) to learn
more.

### SMS

SMS for MFA sends a one-time password to the user's registered mobile phone
number via text message. Read the [Code via SMS](@site/docs/kratos/mfa/30_sms.mdx)
